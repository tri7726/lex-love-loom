...
import { JishoSearch } from '@/components/search/JishoSearch';
...
const studyItems = [
  { path: '/my-classes', icon: GraduationCap, label: 'Khóa học', description: 'Lớp & nhiệm vụ giáo viên giao' },
...
const socialItems = [
  ...
  { path: '/chat', icon: MessageSquare, label: 'Trò chuyện', description: 'Nhắn tin thời gian thực' },
  { path: '/shop', icon: ShoppingIcon, label: 'Sakura Shop', description: 'Đổi XP lấy vật phẩm' },
  { path: '/community-library', icon: Globe, label: 'Thư viện chung', description: 'Chia sẻ kiến thức' },
  { path: '/pet', icon: Cat, label: 'Thú cưng', description: 'Bạn đồng hành học tập' },
...
export const Navigation: React.FC<NavigationProps> = ({
  ...
  return (
    <>
      <header className="sticky top-0 z-50 w-full border-b border-sakura-light/30 bg-cream/80 backdrop-blur-xl supports-[backdrop-filter]:bg-cream/60">
        <div className="container max-w-7xl mx-auto flex h-16 items-center justify-between gap-4 px-4 md:px-8">
          {/* Logo */}
          <Link to="/" className="flex items-center gap-2.5 shrink-0">
            <span className="text-lg">🌸</span>
            <span className="font-jp text-sm font-bold bg-gradient-to-r from-primary to-accent bg-clip-text text-transparent whitespace-nowrap hidden xl:block tracking-wide">
              日本語マスター
            </span>
          </Link>
