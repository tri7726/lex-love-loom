      ...
      <div
        ...
        className={cn(
          ...
          isActive 
            ...
            : "bg-white/40 hover:bg-white text-muted-foreground hover:text-sakura border border-transparent hover:shadow-lg"
        )}
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <p className={cn(
            ...
            isActive ? "text-foreground" : "text-foreground/70 group-hover:text-sakura"
  ...
  return (
    <div className="flex flex-col h-full bg-white p-7 space-y-8 shadow-[20px_0_60px_-15px_rgba(0,0,0,0.03)] z-30">
      <div className="space-y-1 decoration-sakura/20">
        <h3 className="font-serif text-3xl font-black text-foreground tracking-tight">Thư viện</h3>
        <p className="text-[10px] uppercase tracking-[0.3em] font-black text-sakura/40">Lưu ký tri thức</p>
      </div>

      <Button 
        onClick={onNew}
        className="w-full justify-center gap-4 bg-card text-white rounded-[1.5rem] shadow-xl font-black uppercase tracking-[0.2em] text-[11px] h-14 hover:bg-primary/90 hover:scale-[1.02] active:scale-[0.98] transition-all duration-300"
      >
      ...
      <div className="relative group">
        <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground/50 group-focus-within:text-sakura transition-all duration-300" />
        <Input 
          value={searchTerm}
      ...
      <ScrollArea className="flex-1 -mx-2 h-[calc(100vh-320px)]">
        <div className="px-2 space-y-10">
          ...
          <div className="space-y-4">
             <p className="px-4 text-[9px] font-black uppercase tracking-[0.4em] text-muted-foreground/50">Gần đây</p>
             <div className="space-y-2.5">
               {others.length > 0 ? others.map(renderItem) : (
                 <div className="px-4 py-12 text-center space-y-3 opacity-40">
                   ...
                   <p className="text-[11px] font-serif italic text-muted-foreground/70">Vạn sự khởi đầu nan</p>
                 </div>
               )}
             </div>
          </div>
