    ...
    return (
      ...
      >
        ...
        <div>
          ...
          <p className="text-[10px] text-muted-foreground/70 mt-0.5">Đang tạo lộ trình học cá nhân hóa cho bạn</p>
        </div>
      </motion.div>
    );
  }
      ...
      >
        ...
        <div className="mb-3 flex items-start justify-between">
          <div>
            ...
            <h3 className="text-base font-serif font-black text-foreground/80 leading-snug">
              {path.focus_topic}
            </h3>
            <p className="text-[11px] text-muted-foreground/70 mt-0.5 italic">{path.reason}</p>
          </div>
          <Button
            variant="ghost" size="icon"
            className="h-8 w-8 rounded-full text-muted-foreground/50 hover:text-sakura hover:bg-sakura/5 shrink-0"
            onClick={onRefresh}
            ...
            return (
              ...
              >
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[11px] text-muted-foreground mt-0.5 truncate max-w-[160px] group-hover:text-foreground/80">
                    {step.prompt.length > 40 ? step.prompt.slice(0, 40) + '...' : step.prompt}
                  </p>
                </div>
                <ChevronRight className="h-3 w-3 text-muted-foreground/50 group-hover:text-sakura shrink-0 ml-auto transition-colors" />
              </motion.button>
            );
          })}
        </div>
