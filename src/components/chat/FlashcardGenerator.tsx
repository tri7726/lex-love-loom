...
export const FlashcardGenerator = () => {
  ...
  return (
    <div className="space-y-8">
      {/* VIP Generator Card */}
      <Card className="border-0 bg-white/40 backdrop-blur-xl rounded-[2.5rem] shadow-elevated overflow-hidden relative">
        ...
        <CardContent className="p-8 md:p-10 space-y-6 relative z-10">
          <div className="space-y-2 text-center">
            <Badge variant="outline" className="border-indigo-600/30 text-indigo-600 bg-indigo-50/50 font-bold tracking-widest text-[10px] px-3 py-0.5">
              AI VOCABULARY EXTRACTOR
            </Badge>
            <h3 className="font-black text-2xl md:text-3xl text-foreground">
          ...
          <div className="group relative">
            <Textarea
              ...
              className="min-h-[200px] font-jp text-lg leading-relaxed bg-white/70 backdrop-blur-md border-sakura/10 transition-all rounded-3xl p-6 shadow-soft focus-visible:ring-sakura/30"
              value={inputText}
          ...
          <Button
            className="w-full h-14 rounded-2xl text-sm font-black uppercase tracking-widest bg-card text-white shadow-lg transition-all hover:shadow-xl active:scale-[0.98]"
          ...
          >
            <div className="flex flex-col md:flex-row items-center justify-between gap-6 px-4">
              <div className="space-y-1 text-center md:text-left">
                ...
                <p className="text-sm text-muted-foreground font-medium italic">Chọn các thẻ bạn muốn lưu vào Sổ tay để bắt đầu luyện tập.</p>
              </div>
                  ...
                  <Card 
                    className={cn(
                      ...
                      addedCards.has(idx) 
                        ...
                        : 'bg-white/80 backdrop-blur-md hover:border-2 hover:border-sakura/30'
                    )}
                    ...
                    <CardContent className="p-10 space-y-6">
                      <div className="flex justify-between items-start">
                        <div className="space-y-2">
                           <div className="flex items-center gap-3">
                             <div className="text-3xl font-jp font-black text-foreground group-hover:text-sakura transition-colors">{card.word}</div>
                           ...
                           {card.hanviet && (
                             <Badge variant="outline" className="text-[10px] uppercase font-black tracking-widest bg-white text-amber-600 border-amber-200 shadow-sm px-3">
                      ...
                      <div className="space-y-5">
                        <div className="p-4 rounded-2xl bg-white/50 border border-white/20 shadow-inner-soft">
                           <p className="font-bold text-lg text-foreground">{card.meaning}</p>
                        </div>

                        <div className="flex gap-4 p-2">
                           <div className="w-1.5 bg-gradient-to-b from-sakura/60 to-transparent rounded-full" />
                           <div className="space-y-2 py-1 flex-1">
                             <p className="font-jp text-lg leading-relaxed font-bold text-foreground">{card.example_sentence}</p>
                             <p className="text-sm text-muted-foreground italic font-medium">"{card.example_translation}"</p>
                           </div>
                        </div>
                      </div>
      ...
      <AnimatePresence>
        {showFolderPicker && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-50 flex items-center justify-center bg-primary/60 backdrop-blur-md p-4"
          ...
          >
            <motion.div
              ...
              className="bg-white/80 backdrop-blur-2xl rounded-[2.5rem] shadow-elevated-heavy w-full max-w-md p-8 space-y-6 overflow-hidden relative border border-white/20"
            ...
            >
              ...
              <div className="space-y-3 max-h-[280px] overflow-y-auto pr-2 custom-scrollbar">
                {folders.length === 0 && (
                  <div className="text-center py-8 bg-white/40 rounded-3xl border border-dashed border-white/20">
                ...
                {folders.map((folder) => (
                  <button
                    key={folder.id}
                    className="w-full flex items-center gap-4 p-4 rounded-2xl border border-white/40 bg-white/40 hover:border-sakura/50 hover:bg-sakura/5 transition-all text-left shadow-soft hover:shadow-md group"
              ...
              <div className="pt-4 border-t border-white/20 space-y-3">
                ...
                <div className="flex gap-2 bg-white/40 p-1.5 rounded-2xl border border-white/20 shadow-soft">
                  <input
                    value={newFolderName}
