...
const GAME_CARDS: { id: GameMode; label: string; icon: React.FC<{ className?: string }>; desc: string; color: string; bg: string; badge?: string }[] = [
  ...
  { id: 'kanarain',   label: 'Kana Rain',        icon: Gamepad2,    desc: 'Arcade • Từ rơi • Gõ nghĩa',          color: 'text-primary',    bg: 'bg-cyan-50',    badge: 'NEW' },
    ...
    >
      <div className="container max-w-4xl mx-auto py-6 flex-1 flex flex-col">
        ...
        <div className="flex items-center justify-between mb-6">
          <Button variant="ghost" onClick={mode ? () => setMode(null) : onClose} className="gap-2 text-muted-foreground">
          ...
          <div className="w-20 text-right text-xs text-muted-foreground/50 font-medium">{words.length} từ</div>
        </div>
          ...
          {mode ? (
            ...
            <div className="space-y-6">
              <div className="text-center space-y-1">
                <h2 className="text-2xl font-display font-black text-foreground">Chọn chế độ luyện tập</h2>
                <p className="text-sm text-muted-foreground/70 font-medium">{words.length} từ trong bộ sưu tập</p>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                {GAME_CARDS.map((card) => (
                  <button
                    ...
                    className="group relative p-5 rounded-[2rem] bg-white border border-border/50 hover:border-border hover:shadow-[0_12px_30px_-10px_rgba(0,0,0,0.06)] transition-all text-left space-y-3"
                  >
                    {card.badge && (
                    ...
                    <div>
                      <p className="font-bold text-foreground text-sm group-hover:text-foreground">{card.label}</p>
                      <p className="text-[10px] text-muted-foreground/70 font-medium mt-0.5 leading-relaxed">{card.desc}</p>
                    </div>
                  </button>
                ))}
              </div>
