...
export interface FlashcardProps {
  ...
  onRate?: (word: VocabWord, correct: boolean) => void;
}
  ...
  return (
    <div className="space-y-8 py-4">
      <div className="perspective-1000">
        ...
        >
          <div className={cn(
              ...
              <AnimatePresence mode="wait">
                {(reversedCard ? isFlipped : !isFlipped) ? (
                  ...
                  >
                    {isWordSaved && toggleSaved && (
                      <Button
                        ...
                        className="absolute -top-16 right-0 h-10 w-10 text-muted-foreground/40 hover:text-amber-400 active:scale-125 transition-all"
                  ...
                  >
                    ...
                    {/* Mastery indicator */}
                    {getMastery && (() => {
                      const m = getMastery(currentWord.word);
                      if (m === null || m === undefined) return null;
                      return (
                        <div className="mt-4 flex items-center justify-center gap-2">
                          <div className={cn(
                            'flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold',
                            m >= 80 ? 'bg-matcha/10 text-matcha' : m >= 50 ? 'bg-gold/10 text-gold-dark' : 'bg-rose-50 text-rose-500'
                          )}>
                            <div className={cn(
                              'h-1.5 w-1.5 rounded-full',
                              m >= 80 ? 'bg-matcha' : m >= 50 ? 'bg-gold' : 'bg-rose-400'
                            )} />
                            {m >= 80 ? 'Thành thạo' : m >= 50 ? 'Đang học' : 'Cần ôn'}
                          </div>
                        </div>
                      );
                    })()}
            ...
            <div className="absolute inset-x-4 top-1/2 -translate-y-1/2 flex justify-between pointer-events-none">
              <button
                className="pointer-events-auto w-12 h-12 rounded-2xl bg-white/80 border border-border/30 shadow-sm flex items-center justify-center text-muted-foreground/50 hover:text-sakura hover:shadow-md transition-all active:scale-90"
                onClick={onPrev}
              >
        ...
        <p className="text-center text-[10px] text-muted-foreground/50 font-black uppercase tracking-widest mt-6 flex items-center justify-center gap-4">
          <span><kbd className="px-1.5 py-0.5 bg-cream rounded border border-border/50">Space</kbd> Flip</span>
          <span className="w-1 h-1 rounded-full bg-border" />
          <span><kbd className="px-1.5 py-0.5 bg-cream rounded border border-border/50">←</kbd> <kbd className="px-1.5 py-0.5 bg-cream rounded border border-border/50">→</kbd> Nav</span>
        </p>
      </div>
      ...
      <div className="bg-white/50 backdrop-blur-md p-2 rounded-[2rem] border border-white shadow-xl max-w-fit mx-auto">
        <div className="flex items-center gap-1">
          <Button 
            variant="ghost" 
            size="icon" 
            className="h-10 w-10 rounded-xl text-muted-foreground/70 hover:text-sakura" 
            onClick={onReset}
            title="Chơi lại"
          >
          ...
          <div className="w-px h-6 bg-sakura-light/20 mx-1" />
          
          <Button
            variant="ghost" 
            size="icon"
            className="h-10 w-10 rounded-xl text-muted-foreground/70 hover:text-sakura group transition-all"
            onClick={(e) => { e.stopPropagation(); openWritingLab(currentWord.word, getMastery); }}
          ...
          <Button
            variant="ghost" 
            size="icon"
            className={cn('h-10 w-10 rounded-xl transition-all', autoSpeak ? 'bg-rose-100 text-rose-600' : 'text-muted-foreground/70')}
          ...
          <Button
            variant="ghost" 
            size="icon"
            className={cn('h-10 w-10 rounded-xl transition-all', shuffled ? 'bg-rose-100 text-rose-600' : 'text-muted-foreground/70')}
          ...
          <Button
            variant="ghost" 
            size="icon"
            className={cn('h-10 w-10 rounded-xl transition-all', reversedCard ? 'bg-rose-100 text-rose-600' : 'text-muted-foreground/70')}
          ...
          <div className="flex items-center gap-1.5 px-2">
            <Button
              variant="ghost"
              size="icon"
              className="h-9 w-9 rounded-lg text-red-300 hover:bg-red-50 hover:text-red-500 active:scale-90 transition-all"
              onClick={(e) => {
                e.stopPropagation();
                onRate?.(currentWord, false);
                onNext(e);
              }}
              title="Chưa thuộc (X)"
            >
            ...
            <Button
              variant="ghost"
              size="icon"
              className="h-9 w-9 rounded-lg text-emerald-300 hover:bg-emerald-50 hover:text-emerald-500 active:scale-90 transition-all"
              onClick={(e) => {
                e.stopPropagation();
                onRate?.(currentWord, true);
                onNext(e);
              }}
              title="Đã thuộc (✓)"
            >
