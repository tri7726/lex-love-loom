...
interface SeriesViewProps {
  ...
  savedWords: string[]; // array of saved word strings for progress calc
}
                  ...
                  >
                    ...
                    <CardContent className="p-5 text-center space-y-2 relative">
                      ...
                      <div className="relative z-10">
                        ...
                        <div className="mt-3">
                          {(() => {
                            const levelWords = level.lessons.flatMap(l => l.words.map(w => w.word));
                            const learnedCount = savedWords.filter(w => levelWords.includes(w)).length;
                            const pct = levelWords.length > 0 ? Math.round((learnedCount / levelWords.length) * 100) : 0;
                            return (
                              <>
                                ...
                                <p className="text-[10px] text-muted-foreground mt-1">
                                  {learnedCount > 0 ? `${learnedCount}/${levelWords.length} từ` : '0% hoàn thành'}
                                </p>
                              </>
                            );
                          })()}
