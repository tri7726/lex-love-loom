...
import { CheckCircle, XCircle, Trophy, Target, BarChart3, Mic, Headphones, BookOpen } from 'lucide-react';
...
export const SummaryMode: React.FC<SummaryModeProps> = ({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
        ...
        {totalVocabulary > 0 && (
          <Card className="border-none bg-indigo-500/5 shadow-none rounded-2xl">
            <CardContent className="p-5 text-center">
              <BookOpen className="h-5 w-5 mx-auto text-indigo-400 mb-2 opacity-70" />
              <p className="text-2xl font-black">{savedVocabularyCount}/{totalVocabulary}</p>
              <p className="text-[10px] uppercase font-bold text-muted-foreground tracking-widest">Từ vựng</p>
            </CardContent>
          </Card>
        )}
      </div>
      ...
      {/* Grammar Insights */}
      {(() => {
        // Aggregate grammar patterns across all segments
        const grammarCounts = new Map<string, { count: number; explanations: string[] }>();
        segments.forEach(seg => {
          (seg as any).grammar_notes?.forEach((g: { point: string; explanation: string }) => {
            if (!grammarCounts.has(g.point)) {
              grammarCounts.set(g.point, { count: 0, explanations: [] });
            }
            const entry = grammarCounts.get(g.point)!;
            entry.count++;
            if (g.explanation && !entry.explanations.includes(g.explanation)) {
              entry.explanations.push(g.explanation);
            }
          });
        });

        if (grammarCounts.size === 0) return null;

        const sorted = Array.from(grammarCounts.entries()).sort((a, b) => b[1].count - a[1].count);
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-base flex items-center gap-2">
                ...
                Grammar Insights
                <Badge variant="secondary" className="text-[10px] ml-1">{grammarCounts.size} patterns</Badge>
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-2">
                {sorted.map(([point, data]) => (
                  <div key={point} className="flex items-center gap-3 p-2 rounded-lg bg-muted/30 hover:bg-indigo-50/30 transition-colors">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span className="font-jp font-bold text-sm text-foreground">{point}</span>
                        <Badge className="text-[9px] px-1.5 py-0 rounded-full bg-indigo-100 text-indigo-600 border-none">
                          {data.count}x
                        </Badge>
                      </div>
