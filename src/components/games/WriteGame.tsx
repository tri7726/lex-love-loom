...
export const WriteGame: React.FC<WriteGameProps> = ({
        ...
        <Card className="border-0 shadow-2xl overflow-hidden rounded-[2.5rem] bg-gradient-to-br from-indigo-50 via-white to-sky-50">
          ...
          <CardContent className="space-y-8 p-8">
            <div className="text-center relative">
              ...
              <p className="text-muted-foreground/70 font-medium tracking-widest uppercase text-sm mt-2">Độ chính xác</p>
            ...
            <div className="flex flex-col gap-3 pt-2">
              ...
              <div className="flex gap-2">
                <Button 
                  ...
                  className="flex-1 text-muted-foreground hover:bg-cream rounded-2xl py-6"
                >
                  Đổi chế độ
                </Button>
                <Button 
  ...
  const promptClass = writeMode === 'word-to-reading' ? 'text-5xl font-jp font-bold text-foreground' : 'text-3xl font-bold text-foreground leading-tight';
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8">
      ...
      <div className="space-y-2 px-2">
        <div className="flex justify-between items-end mb-1">
          <div className="flex gap-2 items-center">
            ...
            <span className="text-sm font-medium text-muted-foreground/70">Câu {currentIndex + 1} / {questions.length}</span>
          </div>
        ...
        <div className="h-2 w-full bg-sakura-light/20 rounded-full overflow-hidden shadow-inner">
          <motion.div 
      ...
      {/* Main Task Card */}
      <Card className="border-0 shadow-2xl rounded-[3rem] bg-white overflow-hidden border border-border/50">
        <CardContent className="py-20 text-center space-y-6">
          <div className="space-y-4">
            <p className="text-muted-foreground/70 font-medium uppercase tracking-widest text-xs">{promptLabel}</p>
            <p className={cn(promptClass)}>{prompt}</p>

            {/* Extra Context */}
            {writeMode === 'meaning-to-reading' && (
              <div className="bg-cream px-6 py-2 rounded-2xl inline-block border border-border/50">
                <p className="text-lg text-muted-foreground font-jp font-medium">{currentQuestion.word}</p>
              </div>
            )}
            ...
            return (
              <div 
                ...
                className={cn(
                  ...
                  !hasTyped ? "border-border/50 bg-white/50 text-muted-foreground/40" : (isMatch ? "border-green-400 bg-green-50 text-green-600 font-bold" : "border-red-300 bg-red-50 text-red-500")
                )}
              >
                {hasTyped ? typedChar : '?'}
              </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 px-4">
        <div className="relative group">
          <Input
            ...
            className={cn(
              ...
              !showResult ? "border-border focus:border-sky-400 focus:ring-4 focus:ring-sky-50 bg-white" : (isCorrect ? "bg-green-50 border-green-500 text-green-800" : "bg-red-50 border-red-500 text-red-800")
            )}
            disabled={showResult}
            autoComplete="off"
          />
          <div className="absolute left-6 top-1/2 -translate-y-1/2 flex gap-2">
             <Button
                ...
                className={cn(
                  ...
                  showKeyboard ? "bg-sky-100 text-sky-600" : "text-muted-foreground/70 hover:text-sky-500"
                )}
             >
        ...
        <AnimatePresence>
          {showKeyboard && !showResult && (
            ...
            >
              <div className="bg-cream p-6 rounded-[2.5rem] border border-border/50 shadow-inner space-y-4">
                <div className="flex justify-center gap-2 mb-2">
                  <button
                    ...
                    className={cn(
                      ...
                      keyboardType === 'hira' ? "bg-sky-500 text-white" : "bg-white text-muted-foreground/70 border"
                    )}
                  >
                    あ/Hiragana
                  </button>
                  <button
                    ...
                    className={cn(
                      "px-4 py-1 rounded-full text-xs font-bold transition-all",
                      keyboardType === 'kata' ? "bg-sky-500 text-white" : "bg-white text-muted-foreground/70 border"
                    )}
                  >
                    ア/Katakana
                  </button>
                ...
                <div className="flex flex-wrap gap-1.5 justify-center">
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        char === '（' || char === '）' 
                          ...
                          : "bg-white border border-border hover:border-sky-300 hover:bg-sky-50 active:scale-90"
                      )}
                    >
                      {char}
                    </button>
                  ...
                  <button
                    ...
                    className="w-20 h-10 bg-border rounded-xl flex items-center justify-center hover:bg-sakura-light/40 transition-all active:scale-95"
                  >
                    <Delete className="h-5 w-5 text-foreground/70" />
                  </button>
                </div>
              </div>
            </motion.div>
        ...
        {!showResult ? (
          <div className="flex gap-4 justify-center">
            ...
            <button 
              ...
              className="flex-1 max-w-[200px] bg-card text-white rounded-2xl py-5 font-bold shadow-xl hover:bg-primary/90 transition-all active:scale-95 disabled:opacity-50"
            >
              Kiểm tra
            </button>
          </div>
          ...
          >
            ...
            {!isCorrect && (
              <Card className="bg-card text-white border-0 shadow-xl rounded-[2.5rem]">
                <CardContent className="py-8 text-center space-y-3">
                  <p className="text-muted-foreground/70 font-medium uppercase tracking-widest text-[10px]">Đáp án chính xác</p>
                  <p className="text-4xl font-jp font-black text-emerald-400 tracking-wider transition-all">{currentQuestion.reading}</p>
                  <div className="pt-2 flex items-center justify-center gap-2 text-sm text-muted-foreground/50 italic">
            ...
            <div className="flex justify-center gap-4">
              <AISenseiExplain 
                content={currentQuestion.word}
                trigger={
                  <Button
                    variant="outline"
                    className="py-7 px-8 rounded-2xl border-sakura/20 text-sakura hover:bg-sakura/5 gap-3 font-bold shadow-sm"
                  >
              ...
              <Button
                ...
                className="py-7 px-8 rounded-2xl border-border hover:bg-cream gap-3 text-foreground/70 shadow-sm"
              >
