...
export const MultipleChoiceGame: React.FC<MultipleChoiceGameProps> = ({
    ...
    return (
      <div className="max-w-4xl mx-auto space-y-10 py-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
        <div className="text-center space-y-3">
          ...
          <h2 className="text-4xl font-display font-black text-foreground tracking-tight">Cổ Điển - Demon Ops</h2>
          <p className="text-muted-foreground/70 font-medium">Chọn chế độ và cấp độ thử thách</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
          <div className="space-y-4">
            <h3 className="text-xs font-black uppercase tracking-widest text-muted-foreground/70 flex items-center gap-2 px-2">
                ...
                <button
                  ...
                  className={cn(
                    ...
                    subMode === m.id 
                      ...
                      : "border-border/50 bg-white/60 hover:border-sakura/10 hover:bg-white"
                  )}
                >
                  <p className={cn("font-bold transition-colors", subMode === m.id ? "text-sakura" : "text-foreground")}>{m.label}</p>
                  <p className="text-[10px] text-muted-foreground/70 uppercase tracking-wider font-black mt-0.5">{m.desc}</p>
                </button>
              ))}
            </div>
          </div>
              ...
              {[
                ...
                <button
                  ...
                  className={cn(
                    ...
                    difficulty === d.id 
                      ? "border-sakura/20 bg-white shadow-[0_15px_30px_-5px_rgba(255,183,197,0.2)] ring-1 ring-sakura/10" 
                      : "border-border/50 bg-white/60 hover:border-sakura/10"
                  )}
                >
                  <span className="text-2xl mb-1">{d.icon}</span>
                  <p className={cn("text-xs font-black transition-colors", difficulty === d.id ? "text-sakura" : "text-foreground")}>{d.label}</p>
                  <p className="text-[9px] text-muted-foreground/70 uppercase font-black tracking-tighter">{d.desc}</p>
                </button>
              ))}
            </div>
          </div>
        ...
        <div className="flex justify-center pt-8">
           <Button variant="ghost" onClick={onBack} className="rounded-full gap-2 text-muted-foreground/70 hover:text-sakura transition-colors">
      ...
      >
        <Card className="border-0 shadow-2xl overflow-hidden rounded-[3rem] bg-white">
          ...
          <CardHeader className="pb-2">
            <CardTitle className="text-3xl font-display font-bold text-center text-foreground flex items-center justify-center gap-2">
          ...
          <CardContent className="space-y-8 p-10">
            <div className="text-center relative">
              ...
              <p className="text-muted-foreground/70 font-black tracking-widest uppercase text-[10px] mt-2">Chính xác</p>
              <Sparkles className="absolute -top-4 -right-4 h-8 w-8 text-amber-200 animate-pulse" />
            </div>

            <div className="bg-cream p-6 rounded-[2rem] text-center border border-border/50">
            ...
            <div className="flex flex-col gap-3">
              <button 
                onClick={restartGame} 
                className="w-full gap-2 bg-card text-white rounded-[1.5rem] py-6 text-lg font-bold shadow-xl hover:bg-primary/90 transition-all active:scale-95"
              >
              ...
              <Button 
                variant="ghost" 
                onClick={onBack}
                className="w-full text-muted-foreground/70 hover:text-sakura rounded-2xl py-6"
              >
                Quay lại danh sách
              </Button>
            </div>
  ...
  return (
    ...
    >
        ...
        <div className="flex justify-between items-end mb-1">
          <div className="flex items-center gap-3">
            ...
            <Button
              ...
              className="h-8 text-[9px] uppercase tracking-widest font-black text-muted-foreground/50 hover:text-sakura"
            >
          ...
          <div className="flex items-center gap-5">
            {difficulty !== 'peaceful' && (
              <div className={cn(
                ...
                timeLeft < 1.5 ? "border-red-200 text-red-500" : "border-border/30 text-muted-foreground/70"
              )}>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2 bg-white px-4 py-1.5 rounded-full border border-border/30 shadow-sm">
                <CheckCircle2 className="h-3.5 w-3.5 text-emerald-400" />
                <span className="text-sm font-black text-foreground/70">{correctCount}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      >
        <Card className="border-0 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.03)] rounded-[3.5rem] bg-white overflow-hidden border border-border/30/50">
          <CardContent className="py-20 text-center space-y-6">
            <div className={cn("space-y-6 transition-all duration-300", isFlashActive && !showResult && "blur-2xl opacity-0 scale-90")}>
              {isReversed ? (
                <p className="text-4xl font-display font-medium text-foreground px-8 leading-relaxed">{currentQuestion.word.meaning}</p>
              ) : (
                <>
                  <p className="text-6xl font-jp font-black text-foreground tracking-tight">{currentQuestion.word.word}</p>
              ...
              >
                <button
                  className={cn(
                    ...
                    !showResult && "bg-white/80 border-border/50 hover:border-sakura/20 hover:bg-white hover:shadow-xl",
                ...
                >
                  <div className={cn(
                    ...
                    !showResult && "bg-cream text-muted-foreground/50 group-hover:bg-sakura/10 group-hover:text-sakura",
      ...
      {showResult && (
        ...
        >
          <div className="flex items-center gap-3">
            <AISenseiExplain 
              content={currentQuestion.word.word} 
              trigger={
                <button className="flex items-center gap-2 bg-sakura/10 text-sakura px-8 py-5 rounded-[2rem] font-black text-lg shadow-xl hover:bg-sakura/20 transition-all active:scale-95">
            ...
            <button 
              onClick={handleNext} 
              className="flex items-center gap-3 bg-card text-white px-12 py-5 rounded-[2rem] font-black text-lg shadow-2xl hover:bg-primary/90 transition-all active:scale-95 group"
            >
      ...
      {!showResult && (
        <div className="flex justify-center pt-6">
          <Button 
            variant="ghost" 
            onClick={onBack}
            className="text-muted-foreground/50 gap-2 hover:text-sakura rounded-full font-bold px-8"
          >
