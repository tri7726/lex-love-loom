    ...
    <div className="max-w-xl mx-auto space-y-4 py-2">
      ...
      <div className="flex justify-between items-center px-2">
        ...
        <div className="text-center">
          <h2 className="text-2xl font-display font-bold text-foreground">Luyện phát âm</h2>
          <p className="text-xs font-medium text-muted-foreground/70 uppercase tracking-widest">{completedWords.size} / {words.length} hoàn thành</p>
        </div>
        ...
        <CardContent className="space-y-6 p-8 pt-2">
          <div className="text-center space-y-4">
            <div className="space-y-1 relative">
              ...
              <p className="text-4xl font-jp font-bold text-foreground tracking-tight">{currentWord.word}</p>
              {currentWord.reading && (
            ...
            <div className="bg-cream rounded-2xl p-4 border border-border/50 inline-block mx-auto min-w-[200px]">
              <p className="text-xl text-foreground/70 font-medium">{currentWord.meaning}</p>
            </div>
          </div>
          ...
          <div className="flex flex-col items-center gap-8">
            ...
            <div className="flex flex-col items-center gap-5">
              ...
              <p className={cn(
                ...
                isRecording ? "text-red-500 animate-pulse" : "text-muted-foreground/70"
              )}>
                ...
                {lastScore !== null && (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <Badge className={cn("font-bold", getRank(lastScore).color === 'text-emerald-500' ? "bg-emerald-500" : "bg-muted")}>
                        {lastScore}%
                      </Badge>
                    </div>
      ...
      <div className="flex justify-between items-center px-2 pt-2">
        <Button 
          ...
          className="rounded-xl px-4 py-4 text-muted-foreground/70 text-xs"
        >
          <ChevronLeft className="h-4 w-4 mr-1" /> Trước
        </Button>
        <button 
          className="bg-card text-white px-8 py-4 rounded-xl font-bold shadow-xl hover:bg-primary/90 transition-all active:scale-95 flex items-center gap-2 text-sm"
          onClick={handleNext}
        >
