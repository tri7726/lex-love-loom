...
function renderMasked(target: string, maskedIndices: number[], revealedChar?: string): React.ReactNode[] {
  return target.split('').map((ch, i) => {
    ...
    if (maskedIndices.includes(i) && i !== maskedIndices[0]) {
      ...
      return <span key={i} className="text-muted-foreground/50">？</span>;
    }
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between px-2">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-[10px] font-black text-muted-foreground/50 uppercase tracking-widest">Điền ký tự</p>
            <p className="text-sm font-bold text-foreground/80">Câu {currentIndex + 1}/{questions.length}</p>
          </div>
        </div>
      ...
      <motion.div key={currentIndex} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
        <Card className="border-0 shadow-[0_20px_50px_-15px_rgba(0,0,0,0.04)] rounded-[3.5rem] bg-white overflow-hidden border border-border/30">
          <CardContent className="py-14 text-center space-y-5">
            ...
            {/* Word with blank */}
            <div className="text-5xl font-jp font-black text-foreground tracking-wider flex items-center justify-center gap-0.5">
              {displayTarget}
            </div>

            {/* Reading hint if target is kanji word */}
            {currentQ.word.reading && currentQ.target === currentQ.word.word && (
              <p className="text-muted-foreground/50 text-base font-jp">
            ...
            {currentQ.word.example_sentence && (
              <div className="bg-cream rounded-2xl px-5 py-3 text-left mx-4">
                <div className="flex items-start gap-2">
                  <BookOpen className="h-4 w-4 text-muted-foreground/50 mt-0.5 shrink-0" />
                  <div>
                    <p className="text-sm font-jp text-foreground/70 leading-relaxed">{currentQ.word.example_sentence}</p>
                    {currentQ.word.example_translation && (
                      <p className="text-xs text-muted-foreground/70 mt-1 italic">{currentQ.word.example_translation}</p>
                    )}
                  </div>
                </div>
              </div>
            ...
            {/* Audio button */}
            <button onClick={() => speak(currentQ.word.word)} className="inline-flex items-center gap-2 text-xs text-muted-foreground/50 hover:text-violet-500 transition-colors font-medium">
          ...
          {currentQ.options.map((option, idx) => (
            <motion.div key={idx} whileHover={{ y: -3 }} whileTap={{ scale: 0.97 }}>
              <button
                ...
                className="w-full py-8 rounded-[2.5rem] border bg-white border-border/50 hover:border-violet-200 hover:bg-violet-50/30 hover:shadow-xl transition-all text-4xl font-jp font-bold text-foreground/80 hover:text-violet-600"
              >
                {option}
              </button>
            </motion.div>
        ...
        <AnimatePresence>
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
            ...
            <div className={cn('rounded-[2rem] p-6 text-center border', isCorrect ? 'bg-emerald-50 border-emerald-100' : 'bg-red-50 border-red-100')}>
              ...
              <p className="text-sm text-muted-foreground mt-2 font-jp">{currentQ.target}</p>
            </div>
      ...
      <div className="flex justify-center pt-2">
        <Button variant="ghost" onClick={() => { if (window.confirm('Thoát game?')) onBack(); }}
          className="text-muted-foreground/40 hover:text-violet-500 text-sm">
