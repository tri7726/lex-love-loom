...
const ModeSelect: React.FC<{ onSelect: (m: GameMode) => void; onBack: () => void }> = ({ onSelect, onBack }) => (
  <div className="max-w-2xl mx-auto space-y-10 py-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
    <div className="text-center space-y-3">
      <div className="inline-flex items-center justify-center w-16 h-16 rounded-[2rem] bg-white shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-border/30">
        <Zap className="h-8 w-8 text-sakura" />
      </div>
      <h2 className="text-4xl font-display font-black text-foreground tracking-tight">Speed Quiz</h2>
      <p className="text-muted-foreground/70 text-sm font-medium">Chọn chế độ thi đấu</p>
    </div>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
      {MODES.map((m) => (
        <button
          ...
          className="group relative p-7 rounded-[2.5rem] bg-white border border-border/50 hover:border-sakura/30 transition-all hover:shadow-[0_20px_50px_-15px_rgba(0,0,0,0.06)] text-left space-y-4"
        >
          <div className="w-12 h-12 rounded-2xl bg-cream flex items-center justify-center group-hover:bg-sakura/10 transition-all">
            <m.icon className="h-6 w-6 text-muted-foreground/50 group-hover:text-sakura transition-colors" />
          </div>
          <div>
            <h3 className="text-lg font-display font-bold text-foreground group-hover:text-sakura transition-colors">{m.title}</h3>
            <p className="text-[10px] text-muted-foreground/70 uppercase tracking-widest font-black mt-1 leading-relaxed">{m.desc}</p>
          </div>
          <ArrowRight className="absolute right-6 bottom-6 h-5 w-5 text-primary-foreground group-hover:text-sakura group-hover:translate-x-1 transition-all" />
        </button>
      ))}
    </div>

    <div className="flex justify-center">
      <Button variant="ghost" onClick={onBack} className="rounded-full gap-2 text-muted-foreground/50 hover:text-sakura">
        ...
        <Card className="border-0 shadow-2xl overflow-hidden rounded-[3rem] bg-white">
          ...
          <CardContent className="space-y-8 p-10">
            <div className="text-center relative">
              ...
              <p className="text-muted-foreground/70 font-black tracking-widest uppercase text-[10px] mt-2">Tổng điểm</p>
            ...
            <div className="flex justify-center gap-3">
              {[...Array(3)].map((_, i) => (
                <motion.div key={i} initial={{ scale: 0, rotate: -20 }} animate={{ scale: 1, rotate: 0 }} transition={{ delay: i * 0.15, type: 'spring' }}>
                  <Star className={cn('h-10 w-10', i < stars ? 'text-amber-400 fill-amber-400' : 'text-primary-foreground')} />
                </motion.div>
              ))}
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                <div key={i} className="bg-cream p-4 rounded-2xl text-center">
                  <p className="text-2xl font-bold text-foreground">{s.val}</p>
                  <p className="text-[10px] text-muted-foreground/70 uppercase font-black tracking-widest mt-1">{s.label}</p>
                </div>
              ))}
              <div className="col-span-2">
                 <AISenseiExplain 
                   content={questions.map(q => q.word.word).join(' ')}
                   trigger={
                     <Button variant="outline" className="w-full rounded-2xl gap-2 h-12 border-sakura/20 text-sakura hover:bg-sakura/10 font-bold">
                        <Sparkles className="h-4 w-4" /> AI Phân tích bài tập
                     </Button>
                   }
                 />
              </div>
            ...
            <div className="flex flex-col gap-3">
              <button onClick={restart} className="w-full bg-card text-white rounded-[1.5rem] py-5 text-lg font-bold shadow-xl hover:bg-primary/90 transition-all active:scale-95 flex items-center justify-center gap-2">
                <RotateCcw className="h-5 w-5" /> Chơi lại
              </button>
              <Button variant="ghost" onClick={onBack} className="w-full text-muted-foreground/70 hover:text-sakura rounded-2xl">Quay lại danh sách</Button>
            </div>
          </CardContent>
        </Card>
      </motion.div>
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="flex justify-between items-center px-1">
        <div className="flex items-center gap-5">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-2xl font-black text-foreground tracking-tighter">{score.toLocaleString()}</span>
          </div>
          {/* Lives */}
          <div className="flex items-center gap-1">
            {[...Array(3)].map((_, i) => (
              <motion.div key={i} animate={{ scale: i < lives ? [1, 1.2, 1] : 1 }} transition={{ duration: 0.3 }}>
                {i < lives ? <Heart className="h-5 w-5 text-sakura fill-sakura" /> : <HeartOff className="h-5 w-5 text-muted-foreground/40" />}
              </motion.div>
            ))}
          </div>
          {/* Shields */}
          <AnimatePresence>
            {shields > 0 && (
              <motion.div
                ...
                className={cn('flex items-center gap-1 px-3 py-1 rounded-full text-[10px] font-black border', shieldUsed ? 'bg-blue-100 border-blue-300 text-blue-700' : 'bg-cream border-border/50 text-muted-foreground/70')}
              >
        ...
        {/* Timer */}
        <div className={cn('flex items-center gap-2 px-4 py-2 rounded-full border shadow-sm bg-white transition-all', isLowTime ? 'border-red-200 text-red-500' : 'border-border/30 text-muted-foreground/70')}>
      ...
      <motion.div key={currentIndex} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
        <Card className="border-0 shadow-[0_20px_50px_-15px_rgba(0,0,0,0.04)] rounded-[3.5rem] bg-white overflow-hidden border border-border/30">
          <div className="absolute top-6 right-8">
            <Badge variant="outline" className="text-muted-foreground/40 border-border/30 bg-white/50 text-[9px] font-black uppercase tracking-widest">
              {currentIndex + 1} / {questions.length}
            </Badge>
          </div>
          <CardContent className="py-16 text-center space-y-4">
            <p className="text-[10px] text-muted-foreground/50 uppercase tracking-widest font-black">{currentQuestion.promptLabel}</p>
            {isAudio ? (
              <button
              ...
              >
                ...
                <p className="text-xs text-muted-foreground/50 font-medium">Nhấn để nghe lại</p>
              </button>
            ) : (
              <p className={cn(
            ...
            <motion.div key={index} whileHover={!showFeedback ? { y: -3 } : {}} whileTap={!showFeedback ? { scale: 0.98 } : {}}>
              <button
                className={cn(
                  ...
                  !showFeedback && 'bg-white/80 border-border/50 text-foreground/80 hover:border-sakura/20 hover:bg-white hover:shadow-xl',
      ...
      <div className="flex justify-center pt-4">
        <Button variant="ghost" onClick={() => { if (window.confirm('Thoát game?')) onBack(); }}
          className="text-muted-foreground/40 gap-2 hover:text-sakura rounded-full font-bold px-8">
