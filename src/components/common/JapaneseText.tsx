      ...
      <PopoverContent className="w-72 p-6 rounded-[2.5rem] border-0 shadow-[0_20px_50px_-15px_rgba(0,0,0,0.05)] bg-white/95 backdrop-blur-xl" align="center">
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <span className="text-3xl font-jp font-black text-foreground tracking-tighter">{text}</span>
            <Button
              variant="ghost"
              size="icon"
              onClick={speak}
              className="h-10 w-10 bg-cream text-muted-foreground/70 hover:bg-sakura/10 hover:text-sakura rounded-xl transition-all"
            >
          ...
          {furigana && (
            <div className="flex items-center gap-3">
               <Badge variant="outline" className="text-[9px] font-black uppercase text-muted-foreground/50 border-border/50 px-2 py-0">Reading</Badge>
               <p className="text-base text-sakura font-jp font-bold">{furigana}</p>
            </div>
          )}
          {meaning && (
            <div className="pt-4 border-t border-border/30">
              <p className="text-[10px] font-black text-muted-foreground/50 uppercase tracking-widest mb-1.5">Nghĩa tiếng Việt</p>
              <p className="text-base font-display font-medium text-foreground/80 leading-relaxed">{meaning}</p>
            </div>
          )}
          {level && (
