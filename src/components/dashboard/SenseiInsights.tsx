  ...
  const fetchInsights = async (force = false) => {
    ...
    try {
      ...
      toast.error("Sensei không thể phân tích dữ liệu lúc này", {
        description: "Có thể do mạng hoặc dịch vụ AI tạm bận. Hãy thử lại sau.",
        action: { label: 'Thử lại', onClick: () => fetchInsights(true) },
      });
    } finally {
      setLoading(false);
    }
          ...
          <div className="grid md:grid-cols-2 gap-8 items-center">
            <div className="space-y-4">
              <div className="bg-sakura/5 border border-sakura/10 p-5 rounded-3xl relative overflow-hidden">
                ...
                <p className="text-sm font-medium italic text-foreground/80 leading-relaxed mb-4">
              ...
              <div className="space-y-2">
                {data.focus_recommendations.map((rec, i) => (
                  <motion.div 
                    ...
                    className="flex items-start gap-3 p-3 rounded-2xl bg-white border border-border/50 shadow-sm hover:border-sakura/20 transition-colors"
                  >
                    ...
                    <span className="text-[11px] font-medium text-foreground/70">{rec}</span>
                  </motion.div>
                ))}
              </div>
            </div>
             ...
             <h4 className="text-[10px] font-black uppercase tracking-widest text-muted-foreground flex items-center gap-2">
              ...
              <div className="grid sm:grid-cols-3 gap-4">
                {data.weak_areas.map((area, i) => (
                  <div key={i} className="p-4 rounded-2xl bg-cream border border-border/50 space-y-2 relative overflow-hidden group/area">
                    ...
                    <p className="font-jp text-lg font-bold text-foreground">{area.content}</p>
