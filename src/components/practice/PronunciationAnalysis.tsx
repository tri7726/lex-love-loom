...
import { CheckCircle2, XCircle, AlertTriangle, Volume2, TrendingUp, Sparkles, Info } from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      <Card className="shadow-elevated border-2 border-primary/20 overflow-hidden">
        <div className={cn("p-6", getScoreBg(score.ai_evaluation?.score ?? score.overall))}>
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-muted-foreground mb-1">Điểm tổng (AI)</p>
              <div className="flex items-baseline gap-2">
                <span className={cn("text-5xl font-bold", getScoreColor(score.ai_evaluation?.score ?? score.overall))}>
                  {score.ai_evaluation?.score ?? score.overall}
                </span>
                <span className="text-2xl text-muted-foreground">/100</span>
              </div>
              <p className={cn("text-lg font-medium mt-1", getScoreColor(score.ai_evaluation?.score ?? score.overall))}>
                {getScoreLabel(score.ai_evaluation?.score ?? score.overall)}
              </p>
            </div>
            <motion.div
              initial={{ scale: 0 }}
              ...
              className={cn(
                "w-24 h-24 rounded-full flex items-center justify-center border-4 border-white/50",
                getScoreBg(score.ai_evaluation?.score ?? score.overall)
              )}
            >
              <span className={cn("text-4xl font-bold", getScoreColor(score.ai_evaluation?.score ?? score.overall))}>
                { (score.ai_evaluation?.score ?? score.overall) >= 80 ? '🌸' : '💪'}
              </span>
            </motion.div>
          </div>
        </div>
      ...
      {/* AI Phonetic Feedback */}
      {score.ai_evaluation && (
        <Card className="shadow-card border-sakura-light/30 bg-gradient-to-br from-sakura-light/5 to-white">
          <CardHeader className="pb-2">
            <CardTitle className="text-lg flex items-center gap-2">
              <Sparkles className="h-5 w-5 text-sakura" /> Phân tích âm học AI
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <p className="text-sm font-medium leading-relaxed italic text-sumi/80">"{score.ai_evaluation.feedback_vn}"</p>
            
            <div className="space-y-2">
              <p className="text-xs font-black uppercase tracking-widest text-sakura flex items-center gap-2">
                <Info className="h-3 w-3" /> Chi tiết ngữ âm
              </p>
              <div className="grid grid-cols-1 gap-2">
                {score.ai_evaluation.phonetic_tips.map((tip, i) => (
                  <motion.div 
                    key={i} 
                    initial={{ opacity: 0, x: -10 }} 
                    animate={{ opacity: 1, x: 0 }}
                    transition={{ delay: i * 0.1 }}
                    className="flex items-center gap-2 text-sm bg-white/60 p-2 rounded-xl border border-sakura-light/20"
                  >
                    <div className="h-1.5 w-1.5 rounded-full bg-sakura shrink-0" />
                    {tip}
                  </motion.div>
                ))}
              </div>
