    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md bg-white border-border p-0 rounded-2xl flex flex-col max-h-[85vh]">
        <DialogHeader className="p-6 pb-2 text-left">
          <DialogTitle className="text-xl font-serif font-bold text-foreground mb-1">{title}</DialogTitle>
            ...
            <>
              ...
              {recentFolders.length > 0 && !search && (
                <div className="mb-4 shrink-0">
                  <h4 className="text-sm font-serif font-medium mb-2 flex items-center gap-2 text-muted-foreground px-1">
              ...
              <div className="flex flex-col flex-1 min-h-0">
                {!search && <h4 className="text-sm font-serif mb-2 text-muted-foreground px-1">All Folders</h4>}
...
const FolderItem: React.FC<{
  ...
  return (
    <button
      onClick={onSelect}
      className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg border border-transparent bg-transparent hover:bg-cream transition-colors text-left group"
    >
      <div className="flex items-center justify-center w-8 h-8 rounded-md bg-sakura-light/20 text-lg shrink-0">
        {folder.icon || '📁'}
      </div>
      <div className="flex-1 min-w-0">
        <p className="font-medium text-sm truncate group-hover:text-sakura transition-colors text-foreground/80">{folder.name}</p>
      </div>
    </button>
  );
};
