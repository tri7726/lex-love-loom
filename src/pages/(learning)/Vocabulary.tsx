          ...
          <AnimatePresence mode="wait">
            {state.view === 'series' && (
              <SeriesView
                ...
                savedWords={state.savedHistory.map(h => h.word)}
              />
            )}
            ...
            {state.view === 'detail' && state.selectedSeries && state.selectedLevel && state.selectedLesson && (
              <DetailView
                ...
                setSmartReview={setters.setSmartReview}
                getMastery={actions.getMastery}
                // Session tracking
                onGameAnswer={actions.recordAnswer}
                onGameComplete={actions.finishGame}
                showGameStats={state.showGameStats}
                lastGameResult={state.lastGameResult}
                bestStreak={state.bestStreak}
                suggestedNextGame={state.lastGameResult ? actions.suggestNextGame(state.lastGameResult.gameType, state.lastGameResult.wrongWords) : undefined}
                onRetryWrong={actions.handleRetryWrong}
                onTryNextGame={actions.handleTryNextGame}
                onDismissStats={() => { setters.setShowGameStats(false); setters.setActiveGame(null); actions.clearRetry(); }}
                retryWords={state.retryWords}
                customConfig={state.customConfig}
                setCustomConfig={setters.setCustomConfig}
                onAddAllToSRS={actions.addWordsToSRS}
              />
            )}
            ...
            {state.view === 'custom-detail' && state.selectedCustomFolder && (
              <CustomDetailView
                ...
                setCustomConfig={setters.setCustomConfig}
                onAddAllToSRS={actions.addWordsToSRS}
                onExportFolder={actions.exportFolder}
              />
            )}
          </AnimatePresence>
        )}
