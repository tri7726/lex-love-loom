    ...
    return (
      <div>
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            <h2 className="text-lg font-bold text-foreground">Bài {selectedLesson + 1}: {LESSON_NAMES[selectedLesson]}</h2>
            <p className="text-xs text-muted-foreground/70">{currentWords.length} từ · Nhấn thẻ để xem nghĩa</p>
          </div>
        </div>
          ...
          {currentWords.map((word) => {
            ...
            return (
              ...
              >
                {!isFlipped ? (
                  <>
                    <p className="text-2xl font-bold text-foreground mb-1">{word.word}</p>
                    <p className="text-sm text-pink-500">{word.reading}</p>
                    {word.hanviet && <p className="text-xs text-muted-foreground/70 mt-1 italic">{word.hanviet}</p>}
                    <div className="absolute bottom-3 right-4 text-muted-foreground/40 text-xs">nhấn →</div>
                  </>
                ) : (
                  <>
    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
      {LESSON_NAMES.map((name, i) => (
        ...
        >
          ...
          <p className="text-xs text-muted-foreground leading-tight line-clamp-2">{name}</p>
          <div className="absolute bottom-2 right-3 text-xs text-muted-foreground/50 group-hover:text-pink-400 transition-colors">
      ...
      >
        ...
        {image ? (
          <div className="space-y-3">
            ...
            <p className="text-xs text-muted-foreground/70">{imageName} · Nhấn để đổi ảnh</p>
          </div>
        ) : (
          <div className="space-y-3">
            <div className="text-5xl group-hover:scale-110 transition-transform">📷</div>
            <p className="text-muted-foreground font-medium">Kéo thả hoặc nhấn để tải ảnh</p>
            <p className="text-xs text-muted-foreground/70">PNG, JPG, WEBP · Hỗ trợ tiếng Nhật, Hàn, Trung, Anh...</p>
          </div>
        )}
      </div>
      ...
      {image && (
        <div className="bg-white/60 backdrop-blur-sm border border-pink-100 rounded-2xl p-5 space-y-4">
          ...
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-foreground/80">Chế độ</span>
          ...
          {!autoMode && (
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-sm text-foreground/70">Số từ muốn lấy</span>
    ...
    <div className="space-y-5">
      ...
      <div className="relative">
        <textarea
          ...
          rows={6}
          className="w-full bg-white/70 backdrop-blur-sm border border-pink-200 rounded-2xl p-4 pr-14 text-sm text-foreground/80 placeholder-pink-200 focus:outline-none focus:ring-2 focus:ring-pink-300 resize-none"
        />
...
function WordResultGrid({ words, filename }: { words: ExtractedWord[]; filename: string }) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="text-xs bg-white/60 border border-border text-muted-foreground px-3 py-1.5 rounded-full hover:bg-cream transition-all"
          >
            {showRaw ? "Thẻ từ" : "JSON"}
          </button>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <button
          ...
          className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-all ${
            selectedLevel === null
              ...
              : "bg-white/60 text-muted-foreground border-border hover:border-pink-200"
          }`}
        >
          Tất cả
        </button>
        {LEVELS.map((level) => (
          <button
            ...
            className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-all ${
              selectedLevel === level
                ? "bg-muted text-white border-border shadow-sm"
                : "bg-white/60 text-muted-foreground border-border hover:border-pink-200"
            }`}
          >
            {level}
      ...
      {showRaw ? (
        <pre className="bg-card text-green-300 rounded-2xl p-4 text-xs overflow-auto max-h-96">
          ...
          {filtered.map((w, i) => (
            <div key={i} className="relative bg-white/80 backdrop-blur-sm border border-pink-100 rounded-2xl p-4 hover:shadow-md hover:border-pink-200 transition-all">
              ...
              {w.level && LEVEL_COLORS[w.level] ? (
                ...
                <span className="absolute top-2 right-2 text-[10px] text-muted-foreground/50 px-2 py-0.5">
                  —
                </span>
              )}
              <div className="flex items-start justify-between mb-1">
                <p className="text-lg font-bold text-foreground">{w.word}</p>
                {w.language && (
              ...
              <p className="text-sm text-foreground/70 font-medium">{w.meaning}</p>
              {w.part_of_speech && (
                <p className="text-xs text-muted-foreground/70 italic mt-1">{w.part_of_speech}</p>
              )}
              {w.example && (
                <p className="text-xs text-muted-foreground/70 mt-2 pt-2 border-t border-pink-50 line-clamp-2">
                  {w.example}
                </p>
              )}
