      ...
      <main className="max-w-6xl mx-auto px-4 py-12 space-y-8">
        <div className="flex items-center justify-between">
          <div className="space-y-1">
            <Button variant="ghost" size="sm" onClick={() => navigate(-1)} className="gap-2 text-muted-foreground/70 mb-2">
        ...
        <Card className="rounded-[2.5rem] border-2 border-border/50 shadow-xl overflow-hidden">
          <CardHeader className="bg-cream border-b border-border/50 p-8">
            <div className="grid md:grid-cols-3 gap-6">
              <div className="space-y-2">
                <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Tiêu đề đề thi</Label>
              ...
              <div className="space-y-2">
                <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Trình độ</Label>
              ...
              <div className="space-y-2">
                <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Thời gian (Phút)</Label>
          ...
          <CardContent className="p-0">
            <Tabs defaultValue="manual" className="w-full">
              <TabsList className="w-full h-16 bg-white border-b border-border/50 rounded-none p-0">
              ...
              <div className="p-8">
                <TabsContent value="manual" className="m-0 space-y-8">
                  {manualQuestions.map((q, idx) => (
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} key={idx} className="p-8 rounded-[2rem] border-2 border-border/50 space-y-6 relative group">
                       <button 
                         onClick={() => handleRemoveQuestion(idx)}
                         className="absolute top-6 right-6 p-2 text-muted-foreground/50 hover:text-red-500 transition-colors opacity-0 group-hover:opacity-100"
                       >
                       ...
                       <div className="grid md:grid-cols-2 gap-6">
                         <div className="space-y-2">
                            <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/70">Câu hỏi {idx + 1}</Label>
                            <Input value={q.question} onChange={e => {
                               const newQ = [...manualQuestions];
                               newQ[idx].question = e.target.value;
                               setManualQuestions(newQ);
                            }} placeholder="Nhập nội dung câu hỏi..." className="rounded-xl border-border" />
                         </div>
                         <div className="space-y-2">
                            <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/70">Phần thi</Label>
                       ...
                       <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
                          {q.options.map((opt, optIdx) => (
                            <div key={optIdx} className="space-y-2">
                               <Label className="text-[10px] font-black text-muted-foreground/70">Lựa chọn {optIdx + 1}</Label>
                               <div className="relative">
                                  <Input value={opt} onChange={e => {
                                     const newQ = [...manualQuestions];
                                     newQ[idx].options[optIdx] = e.target.value;
                                     setManualQuestions(newQ);
                                  }} className={cn("rounded-xl border-border pr-10", q.correct === optIdx && "border-sakura bg-sakura/5")} />
                                  <button 
                                    onClick={() => {
                                      const newQ = [...manualQuestions];
                                    ...
                                    className={cn("absolute right-3 top-1/2 -translate-y-1/2 rounded-full h-5 w-5 flex items-center justify-center transition-all", q.correct === optIdx ? "bg-sakura text-white" : "bg-sakura-light/20 text-muted-foreground/50")}
                                  >
                       ...
                       <div className="space-y-2">
                          <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/70">Giải thích (Tùy chọn)</Label>
                          <Textarea value={q.explanation} onChange={e => {
                             const newQ = [...manualQuestions];
                             newQ[idx].explanation = e.target.value;
                             setManualQuestions(newQ);
                          }} placeholder="Giải thích đáp án đúng..." className="rounded-xl border-border min-h-[80px]" />
                       </div>

                       <div className="space-y-2">
                          <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/70">Link ảnh minh họa (Tùy chọn)</Label>
                           <Input value={q.image_url} onChange={e => {
                              const newQ = [...manualQuestions];
                              newQ[idx].image_url = e.target.value;
                              setManualQuestions(newQ);
                           }} placeholder="https://image-url.com/photo.jpg" className="rounded-xl border-border" />
                        </div>

                        <div className="space-y-2">
                           <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/70">Đoạn văn đọc hiểu (Dùng cho bài đọc dài)</Label>
                           <Textarea value={q.passage} onChange={e => {
                              const newQ = [...manualQuestions];
                              newQ[idx].passage = e.target.value;
                              setManualQuestions(newQ);
                           }} placeholder="Dán nội dung bài đọc tại đây..." className="rounded-xl border-border min-h-[120px] font-jp text-lg" />
                        </div>
                    </motion.div>
                  ))}
                  
                  <Button variant="outline" onClick={handleAddQuestion} className="w-full h-16 rounded-[1.5rem] border-dashed border-2 border-border hover:border-sakura hover:text-sakura gap-2 font-black">
                ...
                <TabsContent value="json" className="m-0 space-y-6">
                  <div className="space-y-3">
                    <Label className="text-sm font-black text-foreground/80">Dán dữ liệu JSON vào đây</Label>
                    <Textarea 
                      value={jsonText} 
                  ...
                  <Button onClick={handleJsonImport} disabled={!jsonText || isSaving} className="w-full h-14 rounded-2xl bg-muted text-white font-black uppercase tracking-widest">
                ...
                <TabsContent value="sheets" className="m-0 space-y-8">
                  <div className="p-8 rounded-[2.5rem] bg-indigo-50 border-2 border-indigo-100 space-y-6">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="space-y-1">
                        <h4 className="font-black text-foreground">Kết nối Google Sheets</h4>
                  ...
                  <div className="p-6 rounded-2xl border-2 border-border/50 space-y-4">
                    <div className="flex items-center gap-2">
                       <AlertCircle className="h-4 w-4 text-amber-500" />
                       <h5 className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Thứ tự cột CSV chuẩn</h5>
                    </div>
                     <div className="flex gap-2 text-[9px] font-bold">
                        {['Phần thi', 'Câu hỏi', 'Đáp án 1-4', 'Chỉ số đúng (0-3)', 'Giải thích', 'Link ảnh', 'Đoạn văn'].map(h => (
                          <span key={h} className="px-3 py-1 bg-sakura-light/20 rounded-full">{h}</span>
                        ))}
                     </div>
                  </div>
                </TabsContent>
