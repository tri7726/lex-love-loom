...
const SmartReviewQueue = lazy(() => import('@/components/dashboard/SmartReviewQueue').then(m => ({ default: m.SmartReviewQueue })));
...
const GlobalActivityFeed = lazy(() => import('@/components/dashboard/GlobalActivityFeed').then(m => ({ default: m.GlobalActivityFeed })));
...
const SenseiWisdomWidget = lazy(() => import('@/components/dashboard/SenseiWisdomWidget').then(m => ({ default: m.SenseiWisdomWidget })));
...
export const Index = () => {
  ...
  const fetchDueCards = useCallback(async () => {
    if (!user) return;
    const { data: countData } = await supabase.rpc('get_due_flashcards_count', { _user_id: user.id });
    setDueCount((countData as number) || 0);
    const { data } = await (supabase as any)
      .from('flashcards')
      .select('id, word, reading, meaning, next_review_date')
      .eq('user_id', user.id)
  ...
  // Refresh due cards when returning from /review with ?session=done
  useEffect(() => {
    if (searchParams.get('session') === 'done') {
      fetchDueCards();
      // Remove the param from the URL without navigation
      setSearchParams((prev) => { prev.delete('session'); return prev; }, { replace: true });
    }
  }, [searchParams, fetchDueCards, setSearchParams]);

  // Supabase Realtime: watch flashcard updates to refresh dueCount
  useEffect(() => {
    if (!user?.id) return;
    const channel = supabase
      .channel('dashboard-due-cards')
      .on(
        'postgres_changes' as Parameters<ReturnType<typeof supabase.channel>['on']>[0],
        { event: 'UPDATE', schema: 'public', table: 'flashcards', filter: `user_id=eq.${user.id}` },
        () => { fetchDueCards(); }
      )
      .subscribe();
    realtimeRef.current = channel;
    return () => { supabase.removeChannel(channel); };
  }, [user?.id, fetchDueCards]);
  ...
  const handleStartTask = useCallback((taskId: string) => {
    switch (taskId) {
      ...
      case 'ai-chat': navigate('/grammar'); break;
    }
    ...
    >
      ...
      {user?.id && (
        <Suspense fallback={<SuspenseFallback type="card" />}>
          <SectionErrorBoundary name="SmartReviewQueue">
            <SmartReviewQueue />
          </SectionErrorBoundary>
        </Suspense>
      )}

      {/* Sensei Wisdom — static widget, zero API cost */}
      <Suspense fallback={<SuspenseFallback type="card" />}>
        <SectionErrorBoundary name="SenseiWisdom">
          <SenseiWisdomWidget />
        </SectionErrorBoundary>
      </Suspense>
      ...
      <Suspense fallback={<SuspenseFallback type="card" />}>
        <SectionErrorBoundary name="ReviewPreview">
          <ReviewPreview
            dueCards={dueCards}
            dueCount={dueCount}
            onSessionComplete={fetchDueCards}
          />
        </SectionErrorBoundary>
      </Suspense>
      ...
      <Suspense fallback={<SuspenseFallback type="card" />}>
        <SectionErrorBoundary name="GlobalActivityFeed">
          <GlobalActivityFeed />
        </SectionErrorBoundary>
      </Suspense>
