...
const SquadDetail = () => {
  ...
  // Profile cache for Realtime messages (avoids N+1)
  const memberProfileCache = useRef<Record<string, SquadMember['profiles']>>({});
  ...
  const fetchSquadData = useCallback(async () => {
    ...
    try {
      ...
      const { data: memberProfs } = memberIds.length
        ...
        : { data: [] };
      const memberPmap = new Map((memberProfs || []).map((p: any) => [p.user_id, p]));
      const membersData = (rawMembers || []).map((m: any) => ({
        ...m,
        profiles: memberPmap.get(m.user_id) || null,
      }));
      setMembers(membersData);

      // Build profile cache for Realtime messages
      for (const m of membersData) {
        memberProfileCache.current[m.user_id] = m.profiles;
      }
      ...
      const { data: rawMsgs } = await (supabase as any)
        ...
        .limit(50);
      const msgUserIds = Array.from(new Set((rawMsgs || []).map((m: any) => m.user_id)));
      const { data: msgProfs } = msgUserIds.length
        ? await (supabase as any)
            .from('profiles')
            .select('user_id, display_name, avatar_url')
            .in('user_id', msgUserIds as string[])
        : { data: [] };
      const msgPmap = new Map((msgProfs || []).map((p: any) => [p.user_id, p]));
      for (const p of (msgProfs || [])) {
        if (!memberProfileCache.current[p.user_id]) memberProfileCache.current[p.user_id] = p;
      }
      const msgData = (rawMsgs || []).map((m: any) => ({
        ...m,
        profiles: msgPmap.get(m.user_id) || null,
      }));
      setMessages(msgData);
      ...
      .on(
        ...
        (payload) => {
          const newMsg: SquadMessage = {
            ...(payload.new as any),
            profiles: memberProfileCache.current[payload.new.user_id] || null,
          };
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newMessage.trim() || !user || !squadId || !isMember) return;
  ...
  if (!isMember) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center p-8">
        <Card className="max-w-md w-full rounded-[2rem] border-2 border-border/50 text-center p-10 space-y-4">
          <Shield className="h-16 w-16 mx-auto text-muted-foreground/30" />
          <h2 className="text-2xl font-black">Không có quyền truy cập</h2>
          <p className="text-sm text-muted-foreground">Bạn không phải thành viên của biệt đội này.</p>
          <Button onClick={() => navigate('/squads')} className="mt-4">Quay lại</Button>
        </Card>
      </div>
    );
  }
                ...
                {goals.length === 0 ? (
                  ...
                  goals.map(goal => (
                    <div key={goal.id} className="space-y-3">
                      ...
                      <div className="relative h-2 w-full bg-sakura-light/20 rounded-full overflow-hidden">
                        <motion.div 
                          initial={{ width: 0 }}
                    ...
                    return (
                      <div key={msg.id} className={`flex gap-3 ${isOwn ? 'flex-row-reverse' : ''}`}>
                        ...
                        <div className={`space-y-1 max-w-[70%] ${isOwn ? 'items-end' : ''}`}>
                           ...
                           <div className={`px-4 py-2 rounded-2xl text-sm ${isOwn ? 'bg-primary text-white rounded-tr-none' : 'bg-cream text-foreground rounded-tl-none border border-border/50'}`}>
                             {msg.content}
                           </div>
                        </div>
                      </div>
              ...
              <div className="p-4 border-t">
                 <form onSubmit={handleSendMessage} className="flex gap-2">
                    <Input 
                      ...
                      className="rounded-xl bg-cream border-0 focus:ring-primary/20"
                      value={newMessage}
            ...
            <Card className="rounded-[2rem] shadow-sakura border-0 bg-primary text-white overflow-hidden relative">
              ...
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                   <Shield className="h-5 w-5" /> Thành viên tích cực
                </CardTitle>
                <CardDescription className="text-white/70">Tổng XP của các thành viên</CardDescription>
              </CardHeader>
