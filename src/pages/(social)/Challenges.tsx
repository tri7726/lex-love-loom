...
export const Challenges = () => {
  ...
  const attachProfilesToChallenges = useCallback(async (rows: any[]): Promise<Challenge[]> => {
    if (!rows.length) return [];
    const ids = Array.from(new Set(rows.flatMap((r) => [r.challenger_id, r.opponent_id]).filter(Boolean)));
    const { data: profs } = await (supabase as any)
      .from('profiles')
      .select('user_id, display_name, avatar_url, total_xp')
      .in('user_id', ids as string[]);
    const pmap = new Map((profs || []).map((p: any) => [p.user_id, p]));
    return rows.map((r) => ({
      ...r,
      challenger_profile: pmap.get(r.challenger_id) || null,
      opponent_profile: pmap.get(r.opponent_id) || null,
    })) as Challenge[];
  }, []);
  ...
  const fetchChallenges = useCallback(async () => {
    ...
    try {
      ...
      setActiveChallenges(await attachProfilesToChallenges((active as any[]) ?? []));
      ...
      setCompletedChallenges(await attachProfilesToChallenges((completed as any[]) ?? []));
    } catch (err: any) {
  ...
  }, [user, toast, attachProfilesToChallenges]);

  const fetchFriends = useCallback(async () => {
    if (!user) return;
    try {
      // friendships uses sender_id/receiver_id; no FK to profiles
      ...
      const mapped: Friend[] = (profs || []).map((p: any) => ({
        friend_id: p.user_id,
      ...
      <main className="container py-10 space-y-10">
        ...
        <section className="space-y-6">
           <div className="flex items-center justify-between">
              <h2 className="text-xl font-bold flex items-center gap-2 px-1">
                <Sparkles className="h-5 w-5 text-sakura" /> Chế độ chơi đặc biệt
              </h2>
              <Badge className="bg-sakura/10 text-sakura border-sakura/20">NEW EVENT</Badge>
           </div>
           <div className="grid md:grid-cols-2 gap-6">
              ...
              <motion.div 
                ...
                onClick={() => navigate('/kanji-arena')}
              >
                <div className="absolute -inset-1 bg-gradient-to-r from-sakura to-primary rounded-[2.5rem] blur opacity-25 group-hover:opacity-60 transition duration-1000 group-hover:duration-200"></div>
                <Card className="relative h-full border-none rounded-[2.5rem] bg-white overflow-hidden">
                   <div className="absolute top-0 right-0 p-8 opacity-5 rotate-12">
                      <Zap className="h-32 w-32 text-sakura" />
                   </div>
                   <CardContent className="p-8 flex items-center gap-6">
                      <div className="h-20 w-20 rounded-3xl bg-sakura/10 flex items-center justify-center shrink-0">
                         <Zap className="h-10 w-10 text-sakura fill-sakura/20" />
                      </div>
                      <div className="flex-1">
                         <div className="flex items-center gap-2 mb-1">
                            <h3 className="text-xl font-black tracking-tight">Kanji Battle Arena</h3>
                            <Badge className="bg-yellow-400/20 text-yellow-700 text-[9px] font-black border-none">x2 XP</Badge>
                         </div>
                         <p className="text-sm text-muted-foreground leading-relaxed">
                            Thử thách tốc độ! Gõ cách đọc Kanji đang rơi để đạt điểm cao và leo bảng xếp hạng ngày.
                         </p>
                      </div>
                   </CardContent>
                </Card>
              ...
              >
                <div className="absolute -inset-1 bg-gradient-to-r from-rose-500 to-amber-500 rounded-[2.5rem] blur opacity-25 group-hover:opacity-60 transition duration-1000 group-hover:duration-200"></div>
                <Card className="relative h-full border-none rounded-[2.5rem] bg-slate-900 text-white overflow-hidden">
                   <div className="absolute top-0 right-0 p-8 opacity-10 rotate-12">
                      <Skull className="h-32 w-32 text-rose-500" />
                   </div>
                   <CardContent className="p-8 flex items-center gap-6">
                      <div className="h-20 w-20 rounded-3xl bg-rose-500/10 flex items-center justify-center shrink-0">
                         <Skull className="h-10 w-10 text-rose-500" />
                      </div>
                      <div className="flex-1">
                         <div className="flex items-center gap-2 mb-1">
                            <h3 className="text-xl font-black tracking-tight">Boss Battles</h3>
                            <Badge className="bg-rose-500/20 text-rose-300 text-[9px] font-black border-none">LEGENDARY</Badge>
                         </div>
                         <p className="text-sm text-slate-400 leading-relaxed">
                            Đánh bại kẻ gác cổng của mỗi chương học. Trả lời đúng để gây sát thương và nhận vật phẩm hiếm!
                         </p>
                      </div>
                   </CardContent>
                </Card>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <aside className="space-y-6">
            <Card className="border-2 border-sakura/20 shadow-elevated overflow-hidden relative bg-white/40 backdrop-blur-md">
                  ...
                  <div key={i} className="flex items-center justify-between group cursor-pointer hover:bg-sakura/5 p-3 rounded-2xl transition-all border border-transparent hover:border-sakura/10">
                    <div className="flex items-center gap-4">
                      <div className={cn(
                        ...
                        i === 1 ? 'bg-sakura-light/40 text-foreground/80' :
