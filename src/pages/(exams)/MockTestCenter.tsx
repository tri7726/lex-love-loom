...
import { Trophy, Clock, ChevronRight, BarChart, CheckCircle2, AlertCircle, FileText, ArrowLeft, Loader2, Plus, TrendingUp } from 'lucide-react';
...
import { EmptyState, LoadErrorRetry } from '@/components/common';
          ...
          <div className="bg-rose-50/20 p-5 sm:w-48 border-l border-rose-50 flex flex-col justify-center gap-3 text-center relative z-10">
            {completed ? (
              <div className="space-y-3">
                ...
                <Link to={`/mock-tests/${exam.id}`} className="w-full block">
...
export const MockTestCenter = () => {
  ...
  const fetchAll = async () => {
    ...
    if (examRes.error) {
      ...
      setLoadError(examRes.error.message || 'Lỗi không xác định');
      ...
      <main className="container py-4 space-y-6">
                ...
                <div className="p-1 shadow-2xl rounded-[1.6rem] bg-gradient-to-br from-sakura/20 to-crimson/20">
                  <div className="bg-white rounded-2xl p-6 md:p-8 space-y-6">
                    <DialogHeader className="space-y-3 text-center">
                      ...
                      <div>
                        <DialogTitle className="text-2xl font-black text-foreground tracking-tight">Thống kê năng lực JLPT</DialogTitle>
                        <p className="text-muted-foreground/70 font-bold uppercase tracking-widest text-[9px] mt-1">Dữ liệu {results.length} bài thi gần nhất</p>
                      </div>
                    </DialogHeader>
                    <JLPTAnalytics />
                  </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            { icon: Trophy, label: 'Điểm cao nhất', val: loading ? null : (bestScore > 0 ? `${bestScore}/180` : '—'), color: 'bg-amber-500', bg: 'bg-amber-50 border-amber-200/30' },
        ...
        {loading ? (
          ...
          <LoadErrorRetry
            ...
            description="Có thể do mạng yếu hoặc máy chủ tạm bận. Hãy thử lại."
            error={loadError}
            onRetry={fetchAll}
          />
          ...
          <EmptyState
            emoji="📝"
            title={`Chưa có đề thi nào${selectedLevel ? ` cho ${selectedLevel}` : ''}`}
            description="Đề thi mới đang được biên soạn. Trong lúc chờ, bạn có thể luyện tập với flashcard hoặc đọc bài."
            actionLabel="Luyện flashcard"
            onAction={() => (window.location.href = '/srs-review')}
          />
        ) : (
        ...
        <Card className="border-sakura/30 bg-sakura/5 rounded-3xl overflow-hidden relative">
          ...
          <CardContent className="p-8 flex items-start gap-6 relative z-10">
            ...
            <div className="space-y-1">
              <h4 className="font-bold text-lg text-foreground">Lưu ý khi làm đề thi thử</h4>
