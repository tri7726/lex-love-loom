...
export const JLPTMockExam = () => {
  ...
  const [instantFeedback, setInstantFeedback] = useState<{ isCorrect: boolean; correctIdx: number } | null>(null);
  const [feedbackQuestionIdx, setFeedbackQuestionIdx] = useState<number | null>(null);

  // Power-up state
  const [powerUps, setPowerUps] = useState({ fiftyFifty: 2, doublePoints: 1 });
  const [eliminatedOptions, setEliminatedOptions] = useState<Set<number>>(new Set());
  const [doublePointsActive, setDoublePointsActive] = useState(false);

  // Review filter state
  const [reviewFilter, setReviewFilter] = useState<'all' | 'wrong' | 'section'>('all');
  ...
  // Sync currentSection with currentIdx
  useEffect(() => {
    if (questions.length > 0 && currentIdx >= 0 && currentIdx < questions.length) {
      const qSection = questions[currentIdx].section_type;
      if (qSection !== currentSection) {
        setCurrentSection(qSection as any);
      }
    }
  }, [currentIdx, questions, currentSection]);

  // Timer Logic
  useEffect(() => {
    if (loadingExam || isFinished || !meta || questions.length === 0) return;
  ...
  }, [loadingExam, isFinished, currentSection, meta, questions.length]);
  ...
  }, [sectionTimeLeft, currentSection, loadingExam, isFinished, meta, questions.length]);
  ...
  const handleSectionTimeout = () => {
    if (currentSection === 'vocabulary_grammar') {
      ...
      if (firstReadingIdx !== -1) {
        setCurrentIdx(firstReadingIdx);
      } else {
      ...
      const firstListeningIdx = questions.findIndex(q => q.section_type === 'listening');
      if (firstListeningIdx !== -1) {
        setCurrentIdx(firstListeningIdx);
      } else {
        handleFinish();
      }
    } else {
  ...
  const handleFinish = async () => {
    ...
    setSaving(true);
    const { data: prevResults } = await (supabase as any)
    ...
    if (prevResults && prevResults.length > 0) {
      setPreviousBestScore(prevResults[0].score);
    }
    ...
    // XP reward for completing exam
    if (totalEarnedXP > 0) {
      awardXP('quiz', totalEarnedXP, { exam_id: meta.id, type: 'mock_exam_total' });
    }

    // Confetti on pass
    if (overallPassed) {
      setTimeout(() => fireConfetti('school'), 300);
      setTimeout(() => fireConfetti('sakura'), 800);
    }

    // --- SRS Sync: Vocabulary + Grammar ---
    // Sync vocabulary questions
    ...
    // Sync grammar questions too (extract grammar patterns from options)
    for (const q of questions) {
      if (q.section_type !== 'vocabulary_grammar') continue;
      const idx = questions.indexOf(q);
      const isCorrect = answers[idx] === q.correct;
      // Try to extract Japanese text from options for grammar SRS
      for (const opt of q.options) {
        const jpMatch = opt.match(/[぀-ゟ゠-ヿ一-龯]+/);
        if (jpMatch) {
          syncQuizResult(jpMatch[0], isCorrect).catch(console.error);
          break;
        }
      }
    }
  ...
  const handleSelectOption = (idx: number) => {
    if (instantFeedback) return; // Block re-selection during feedback
    if (isReviewMode) return;
    if (answers[currentIdx] !== undefined) return; // Already answered

    const isCorrect = idx === currentQ.correct;
    setAnswers(prev => ({ ...prev, [currentIdx]: idx }));
    setInstantFeedback({ isCorrect, correctIdx: currentQ.correct });
    setFeedbackQuestionIdx(currentIdx);
    setEliminatedOptions(new Set()); // Reset eliminated on new question
    ...
    if (isCorrect) {
      ...
      // Score
      const points = (currentQ.point_weight || 2) * (doublePointsActive ? 2 : 1);
      setLiveScore(prev => prev + points);
      if (doublePointsActive) setDoublePointsActive(false);

      // XP
      const xpAmount = 5 + Math.min(newStreak, 10); // 5 base + up to 10 streak bonus
      setTotalEarnedXP(prev => prev + xpAmount);
      awardXP('quiz', xpAmount, { exam_id: meta?.id, question_idx: currentIdx, streak: newStreak });
      showXPGain(xpAmount, `Đúng x${newStreak}`);

      // Streak milestones
      if (newStreak === 5) {
        sonnerToast.success(`🔥 Streak x5! Tiếp tục phát huy!`);
        fireConfetti('success');
      }
      if (newStreak === 10) {
        sonnerToast.success(`🔥🔥 Streak x10! Bạn đang hủy diệt!`);
        fireConfetti('success');
      }
    } else {
      setStreak(0);
    ...
    // Auto-advance after 800ms if not last question
  ...
  const reviewFilteredIndices = useMemo(() => {
    if (!isReviewMode) return null;
    if (reviewFilter === 'wrong') {
      return questions.map((q, i) => answers[i] !== q.correct ? i : -1).filter(i => i !== -1);
    }
    if (reviewFilter === 'section') {
      return questions.map((q, i) => q.section_type === reviewSectionFilter ? i : -1).filter(i => i !== -1);
    }
    return questions.map((_, i) => i);
  }, [isReviewMode, reviewFilter, reviewSectionFilter, questions, answers]);

  const nextQuestion = () => {
    // Clear instant feedback state if active
    if (instantFeedback) {
      setInstantFeedback(null);
      setFeedbackQuestionIdx(null);
    }
    if (isReviewMode && reviewFilteredIndices) {
      const currentFilterIdx = reviewFilteredIndices.indexOf(currentIdx);
      if (currentFilterIdx < reviewFilteredIndices.length - 1) {
        setCurrentIdx(reviewFilteredIndices[currentFilterIdx + 1]);
      }
      return;
    }
    if (currentIdx < questions.length - 1) {
        ...
        <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-sakura/10 px-6 py-4 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div className="hidden sm:block text-left">
              ...
              <h4 className="text-sm font-bold text-foreground line-clamp-1">{meta?.title}</h4>
            </div>
          </div>
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-6 text-center overflow-y-auto custom-scrollbar bg-sakura-light/5">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-xl w-full space-y-6 py-8">
            ...
            <div className="space-y-2">
              <h1 className="text-3xl sm:text-4xl font-black font-display text-foreground tracking-tight">Kết quả bài thi</h1>
              <div className="flex items-center justify-center gap-2">
                ...
                <p className="text-muted-foreground/70 font-bold uppercase tracking-[0.2em] text-[9px]">{meta?.title}</p>
              </div>
            </div>
            ...
            <div className="flex justify-center gap-2">
              {[1, 2, 3].map(star => {
                const pct = total / 180;
                const filled = star === 1 ? pct >= 0.3 : star === 2 ? pct >= 0.6 : pct >= 0.8;
                return (
                  <motion.span
                    key={star}
            ...
            <div className="grid grid-cols-3 gap-3">
              {Object.entries(sectionResults).map(([key, res]) => (
                <Card key={key} className={cn("p-4 rounded-[1.5rem] border shadow-sm", res.passed ? "border-emerald-100 bg-emerald-50/20" : "border-rose-100 bg-rose-50/20")}>
                  <p className="text-[8px] font-black text-muted-foreground/70 uppercase tracking-widest mb-1">{key.replace('_', ' & ')}</p>
                  <div className="text-lg font-black text-foreground">{res.score}<span className="text-[10px] text-muted-foreground/70">/{res.max}</span></div>
            ...
            <Card className="border-[3px] border-sakura rounded-[2.5rem] shadow-lg overflow-hidden bg-white">
              <CardContent className="p-8 space-y-6">
                <div className="flex flex-col items-center">
                  <p className="text-[9px] font-black text-muted-foreground/70 uppercase tracking-[0.3em] mb-1">TỔNG ĐIỂM CHUẨN HÓA</p>
                  <h2 className="text-5xl font-black text-crimson leading-none">{total}<span className="text-sm text-muted-foreground/50 ml-1">/180</span></h2>
                </div>

                <div className="py-4 px-8 rounded-2xl bg-cream border border-border/50 flex items-center justify-around">
                  <div className="text-center">
                    <p className="text-lg font-black text-foreground">{Math.round((total / 180) * 100)}%</p>
                    <p className="text-[8px] text-muted-foreground/70 font-black uppercase">Tỉ lệ đúng</p>
                  </div>
                  <div className="h-6 w-px bg-border" />
                  <div className="text-center">
                    <p className="text-lg font-black text-foreground">{formatTime(timeTaken)}</p>
                    <p className="text-[8px] text-muted-foreground/70 font-black uppercase">Thời gian</p>
                  </div>
                  {totalEarnedXP > 0 && (
                    <>
                      <div className="h-6 w-px bg-border" />
                      <div className="text-center">
                        <p className="text-lg font-black text-amber-500">+{totalEarnedXP}</p>
                        <p className="text-[8px] text-muted-foreground/70 font-black uppercase">XP</p>
                      </div>
                    </>
                  )}
                  {maxStreak >= 2 && (
                    <>
                      <div className="h-6 w-px bg-border" />
                      <div className="text-center">
                        <p className="text-lg font-black text-orange-500">🔥{maxStreak}</p>
                        <p className="text-[8px] text-muted-foreground/70 font-black uppercase">Streak</p>
                      </div>
                    </>
                  )}
                </div>
                ...
                {previousBestScore !== null && (
                  <div className="py-2 px-4 rounded-xl bg-indigo-50 border border-indigo-100">
                    <p className="text-xs text-indigo-600 font-bold text-center">
                      {total > previousBestScore
                        ? `🎉 Cải thiện ${total - previousBestScore} điểm so với lần trước!`
                        : total === previousBestScore
                          ? `📊 Bằng điểm với lần thi trước (${previousBestScore})`
                          : `📉 Thấp hơn ${previousBestScore - total} điểm so với lần trước. Cố gắng hơn nhé!`
                      }
                    </p>
                  </div>
                )}
                ...
                {Object.entries(sectionResults).length > 0 && (
                  <div className="space-y-2">
                    <p className="text-[9px] font-black text-muted-foreground/70 uppercase tracking-[0.2em]">Phân tích kỹ năng</p>
                    {Object.entries(sectionResults).map(([key, res]) => (
                      <div key={key} className="space-y-1">
                        <div className="flex justify-between text-[10px] font-bold">
                          <span className="text-foreground/70">{key.replace('_', ' & ')}</span>
                          <span className={res.passed ? "text-emerald-600" : "text-rose-600"}>{res.score}/{res.max}</span>
                        </div>
                        <div className="h-2 bg-sakura-light/20 rounded-full overflow-hidden">
                          <motion.div
                            initial={{ width: 0 }}
                            animate={{ width: `${(res.score / res.max) * 100}%` }}
                            transition={{ duration: 0.8, delay: 0.5 }}
                            className={cn("h-full rounded-full", res.passed ? "bg-emerald-400" : "bg-rose-400")}
                          />
                        </div>
                      </div>
                    ))}
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <Button variant="ghost" className="flex-1 h-14 rounded-2xl font-black uppercase tracking-widest text-muted-foreground/70" onClick={() => navigate('/learning-path')}>Trang chủ học tập</Button>
            </div>
          </motion.div>
        </div>
      </div>
  ...
  if (questions.length === 0) return (
    <div className="min-h-screen bg-sakura-light/20 flex flex-col items-center justify-center p-6">
      <div className="max-w-md w-full bg-white/80 backdrop-blur-lg p-10 rounded-[3rem] border border-sakura shadow-soft text-center space-y-6">
        ...
        <div className="space-y-2">
          <h2 className="text-2xl font-bold font-display text-foreground">Đề thi đang trống</h2>
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-4">
          <div className="flex items-center gap-4 min-w-0">
            ...
            <div className="min-w-0">
              <h1 className="text-lg font-black text-foreground tracking-tight truncate">{meta?.title || 'JLPT Mock Exam'}</h1>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-black text-muted-foreground/70 uppercase tracking-widest whitespace-nowrap">
             ...
             {!isReviewMode && (
               <>
                 <Button
                    ...
                    className={cn("rounded-xl h-9 w-9 transition-all", flagged.has(currentIdx) ? "bg-amber-100 text-amber-600 shadow-inner" : "bg-cream text-muted-foreground/70")}
                 >
                   <BookOpen className={cn("h-4 w-4", flagged.has(currentIdx) && "fill-amber-600")} />
                 </Button>
                 {streak >= 2 && (
                   <div className="hidden sm:flex items-center gap-1 px-2.5 py-1.5 rounded-xl bg-gradient-to-r from-orange-50 to-red-50 border border-orange-200 font-black text-xs text-orange-600 shadow-sm">
                     <span>🔥</span>
                     <span className="font-mono">{streak}</span>
                   </div>
                 )}
                 {liveScore > 0 && (
                   <div className="hidden sm:flex items-center gap-1 px-2.5 py-1.5 rounded-xl bg-gradient-to-r from-sakura/10 to-crimson/10 border border-sakura/20 font-black text-xs text-crimson shadow-sm">
                     <span>⭐</span>
                     <span className="font-mono">{liveScore}</span>
                   </div>
                 )}
                 <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-xl bg-cream border border-border/50 font-black text-[9px] text-muted-foreground/70 uppercase tracking-widest">
            ...
            <div className="max-w-2xl mx-auto space-y-6">
              {currentQ.audio_url && (
                <div className="bg-white p-8 rounded-[2.5rem] border-4 border-purple-100 shadow-2xl space-y-6 text-center">
                  ...
                  <div className="space-y-2">
                    <h4 className="text-xl font-black text-foreground italic uppercase">Listening Track</h4>
                    <p className="text-xs text-muted-foreground/70 font-bold tracking-widest uppercase">Chỉ được nghe duy nhất 01 lần</p>
                  </div>
                  
                  {playedAudios.has(currentQ.id) ? (
                    <div className="p-6 bg-cream rounded-2xl border-2 border-dashed border-border text-muted-foreground/70 font-jp font-bold flex items-center justify-center gap-3">
              ...
              {currentQ.passage && (
                ...
                >
                  ...
                  <div className="relative z-10 whitespace-pre-wrap font-jp text-[1.35rem] leading-[2.2] text-foreground/80 font-medium">
                    {currentQ.passage}
                  </div>
                </motion.div>
              )}
          ...
          <div className="max-w-xl w-full space-y-6 py-2">
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-muted-foreground/70 font-black text-[9px] uppercase tracking-[0.2em]">CÂU HỎI HIỆN TẠI</span>
              </div>
              <h2 className={cn(
                "font-jp font-black text-foreground leading-tight",
            ...
            {/* Review Mode Filter Chips */}
            {isReviewMode && (
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-[10px] font-bold text-muted-foreground/70 uppercase tracking-widest mr-1">Lọc:</span>
                {(['all', 'wrong', 'section'] as const).map(filter => (
                  <Button
                    key={filter}
                    ...
                    onClick={() => {
                      ...
                      // Jump to first matching question
                      const newIdx = filter === 'all' ? 0 :
                        filter === 'wrong' ? questions.findIndex((q, i) => answers[i] !== q.correct) :
                        filter === 'section' ? questions.findIndex(q => q.section_type === reviewSectionFilter) :
                        0;
                      if (newIdx !== -1) setCurrentIdx(newIdx);
                    }}
                    className={cn(
                      "rounded-xl text-[10px] h-8 px-3 font-bold",
                      reviewFilter === filter ? "bg-crimson text-white" : "border-border text-muted-foreground"
                    )}
                  >
                    {filter === 'all' ? 'Tất cả' : filter === 'wrong' ? 'Chỉ câu sai' : 'Theo phần'}
                  </Button>
                ))}
                {reviewFilter === 'section' && (
                  <select
                    value={reviewSectionFilter}
                    onChange={(e) => {
                      setReviewSectionFilter(e.target.value);
                      const newIdx = questions.findIndex(q => q.section_type === e.target.value);
                      if (newIdx !== -1) setCurrentIdx(newIdx);
                    }}
                    className="h-8 rounded-xl text-[10px] font-bold border border-border bg-white px-2 text-foreground/70"
                  >
                    {['vocabulary_grammar', 'reading', 'listening'].map(s => (
            ...
            {/* Power-ups */}
            {!isReviewMode && !instantFeedback && answers[currentIdx] === undefined && (
              <div className="flex items-center gap-2">
                <Button
                  variant="outline"
                  size="sm"
                  ...
                  onClick={() => {
                    ...
                    const incorrect = currentQ.options
                      .map((_, i) => i)
                    ...
                    sonnerToast.success('Đã loại 2 đáp án sai!');
                  }}
                  className="rounded-xl text-[10px] h-8 gap-1.5 font-bold border-amber-200 text-amber-600 hover:bg-amber-50"
                >
                ...
                <Button
                  variant="outline"
                  size="sm"
                  disabled={doublePointsActive || powerUps.doublePoints <= 0}
                  onClick={() => {
                    if (powerUps.doublePoints <= 0) return;
                    setDoublePointsActive(true);
                    setPowerUps(prev => ({ ...prev, doublePoints: prev.doublePoints - 1 }));
                    sonnerToast.success('x2 Điểm cho câu tiếp theo!');
                  }}
                  className={cn(
                    "rounded-xl text-[10px] h-8 gap-1.5 font-bold",
                    doublePointsActive
                      ? "bg-purple-100 border-purple-300 text-purple-700"
                      : "border-purple-200 text-purple-600 hover:bg-purple-50"
                  )}
                >
                  🚀 x2 Điểm ({powerUps.doublePoints})
                </Button>
                {doublePointsActive && (
                  <span className="text-[10px] font-bold text-purple-600 bg-purple-50 px-2 py-1 rounded-lg animate-pulse">
                    x2 ĐANG HOẠT ĐỘNG
                  </span>
                )}
              </div>
            ...
            <div className="grid gap-3 w-full">
              {currentQ.options.map((option, idx) => {
                ...
                const showFeedback = instantFeedback && feedbackQuestionIdx === currentIdx;
                const isEliminated = eliminatedOptions.has(idx);

                let variantClass = "bg-white border-border/50 hover:border-sakura/20 text-foreground/70 hover:bg-sakura/5";
                let iconClass = "bg-cream border-transparent text-muted-foreground/70 group-hover:bg-sakura/10 group-hover:text-sakura";

                if (showFeedback) {
                  if (isCorrect) {
                    variantClass = "bg-emerald-500 border-emerald-500 text-white shadow-lg shadow-emerald-200/50 scale-[1.02]";
                    iconClass = "bg-white/20 border-white/40 text-white rotate-12";
                  } else if (isSelected && !isCorrect) {
                    variantClass = "bg-rose-500 border-rose-500 text-white shadow-lg shadow-rose-200/50 animate-[shake_0.3s_ease-in-out]";
                    iconClass = "bg-white/20 border-white/40 text-white -rotate-12";
                  } else {
                    variantClass = "bg-white border-border/50 text-muted-foreground/50 opacity-40";
                    iconClass = "bg-cream border-transparent text-muted-foreground/40";
                  }
                } else if (isEliminated) {
                  variantClass = "bg-cream border-dashed border-border text-muted-foreground/50 line-through opacity-40";
                  iconClass = "bg-sakura-light/20 border-transparent text-muted-foreground/40";
                  ...
                  if (isCorrect) {
                    ...
                    variantClass = "bg-white border-border/50 text-muted-foreground/50 opacity-50";
                    iconClass = "bg-cream border-transparent text-muted-foreground/40";
                  }
                } else if (isSelected) {
                ...
                return (
                  <motion.button
                    ...
                    onClick={() => !isReviewMode && !showFeedback && handleSelectOption(idx)}
                    disabled={isReviewMode || showFeedback || isEliminated}
                    className={cn(
            ...
            {isReviewMode && currentQ.explanation && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="p-4 rounded-xl bg-cream border-2 border-border/50 space-y-2">
                <div className="flex items-center gap-2">
                  <div className="h-7 w-7 bg-amber-100 rounded-lg flex items-center justify-center text-amber-600">
                    <BookOpen className="h-3.5 w-3.5" />
                  </div>
                  <h4 className="font-black text-foreground text-[10px] uppercase tracking-widest">Giải thích</h4>
                </div>
                <p className="text-foreground/70 font-jp leading-relaxed whitespace-pre-wrap text-[13px]">{currentQ.explanation}</p>
              </motion.div>
            )}
          </div>
          </div>
              ...
              {questions.map((q, idx) => (
                <button key={idx} onClick={() => setCurrentIdx(idx)}
                  className={cn(
                    ...
                    isReviewMode 
                      ? (answers[idx] === q.correct ? "bg-emerald-500 text-white" : answers[idx] === undefined ? "bg-sakura-light/20 text-muted-foreground/50" : "bg-rose-500 text-white")
      ...
      <footer className="sticky bottom-0 bg-white/90 backdrop-blur-md border-t border-sakura/10 px-6 py-5 shrink-0">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-6">
          ...
          <div className="hidden md:flex flex-1 max-w-sm flex-col gap-2">
            <div className="flex justify-between items-center text-[10px] font-black text-muted-foreground/70 uppercase tracking-widest">
              <span>Tiến độ hoàn thành</span>
              <span>{Math.round(((currentIdx + 1) / questions.length) * 100)}%</span>
            </div>
            <div className="h-2.5 w-full bg-sakura-light/20 rounded-full overflow-hidden p-0.5">
              <motion.div 
                initial={{ width: 0 }}
            ...
            >
              <div className="p-8 border-b flex items-center justify-between shrink-0 bg-sakura/5">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-black text-foreground">Danh sách câu</h3>
                  ...
                  {questions.map((q, idx) => (
                    <button key={idx} onClick={() => { setCurrentIdx(idx); setIsSidebarOpen(false); }}
                      className={cn(
                        ...
                        isReviewMode 
                          ? (answers[idx] === q.correct ? "bg-emerald-500 text-white" : answers[idx] === undefined ? "bg-border text-muted-foreground/70" : "bg-rose-500 text-white")
                  ...
                  {isReviewMode ? (
                    ...
                    <>
                      <div className="flex items-center gap-3 bg-white p-3 rounded-2xl border border-sakura/10">
                        ...
                        <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">Đã trả lời</span>
                      </div>
                      <div className="flex items-center gap-3 bg-white p-3 rounded-2xl border border-sakura/10">
                        <div className="w-4 h-4 rounded-[6px] bg-sakura/20" />
                        <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">Chưa làm</span>
                      </div>
                    </>
                  )}
                </div>
      ...
      <Dialog open={showSubmitDialog} onOpenChange={setShowSubmitDialog}>
        <DialogContent className="rounded-[3rem] p-12 border-none shadow-[0_32px_64px_-12px_rgba(0,0,0,0.14)]">
          <DialogHeader className="space-y-4 text-center">
            ...
            <DialogTitle className="text-3xl font-black text-foreground">Xác nhận nộp bài?</DialogTitle>
      ...
      <Dialog open={showExitConfirm} onOpenChange={setShowExitConfirm}>
        <DialogContent className="rounded-[3rem] p-12 border-none shadow-[0_32px_64px_-12px_rgba(0,0,0,0.14)]">
          <DialogHeader className="space-y-4 text-center">
            ...
            <DialogTitle className="text-3xl font-black text-foreground">Thoát bài thi?</DialogTitle>
            <DialogDescription className="text-muted-foreground/70 font-medium">
