...
export const useFriends = () => {
  ...
  const fetchFriends = useCallback(async (reset = true) => {
    ...
    try {
      ...
      const friendIds = (data || []).map((r: any) => r.receiver_id).filter(Boolean);
      let formattedFriends: FriendProfile[] = [];
      if (friendIds.length > 0) {
        const { data: profiles } = await (supabase as any)
          .from('profiles')
          .select('user_id, display_name, avatar_url, jlpt_level, total_xp')
          .in('user_id', friendIds);
        formattedFriends = (profiles || []) as FriendProfile[];
      }
  ...
  const followUser = async (friendId: string, profileMeta?: Partial<FriendProfile>) => {
    ...
    try {
      const { error } = await (supabase as any)
        .from('friendships')
        .insert({ sender_id: user.id, receiver_id: friendId, status: 'accepted' });
