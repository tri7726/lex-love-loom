...
export const useFSRS = () => {
  ...
  const reviewCard = useCallback(async (flashcardId: string, rating: FSRSRating, providedCard?: any) => {
    ...
    if (!card.fsrs_state || card.fsrs_state === 'new') {
      ...
      // Subsequent review
      // Per FSRS-4: stability update uses PREVIOUS difficulty, not the newly computed one
      const t = card.interval ?? 1;
      const prevD = card.fsrs_difficulty ?? 5;
      const prevS = card.fsrs_stability ?? 1;
      const r = forgettingCurve(t, prevS);
      d = updateDifficulty(prevD, rating);
      s = updateStability(prevD, prevS, r, rating);
    ...
    const updateFields = {
      ...
      ease_factor: Math.max(1.3, 2.5 - (d - 5) * 0.15),
    };
      ...
      for (const action of queue) {
        try {
          if (action.type === 'REVIEW') {
            // Staleness check: skip if card was reviewed online after queue timestamp
            const { data: card } = await (supabase as any)
              .from('flashcards')
            ...
            if (card?.last_reviewed_at && new Date(card.last_reviewed_at) > new Date(action.timestamp)) {
