...
export const useWordHistory = () => {
  ...
  const fetchHistory = useCallback(async () => {
    ...
    try {
      ...
      setHistory((data as WordHistoryItem[]) || []);
    } catch (err) {
  ...
  const saveWord = useCallback(async (word: { 
    ...
    try {
      const { error } = await (supabase as any).from('saved_vocabulary').upsert({
        user_id: user.id,
        word: word.word,
  ...
  // Update mastery level for a saved word
  const updateMastery = useCallback(async (wordText: string, correct: boolean): Promise<boolean> => {
    if (!user) return false;
    const item = history.find(h => h.word === wordText);
    if (!item) return false;

    const delta = correct ? 10 : -15;
    const current = item.mastery_level ?? 0;
    ...
    try {
      ...
      setHistory(prev => prev.map(h => h.id === item.id ? { ...h, mastery_level: newMastery } : h));
      return true;
    } catch (err) {
