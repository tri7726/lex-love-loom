  ...
  const generatePath = useCallback(async () => {
    ...
    try {
      ...
      if (!knowledge || knowledge.length < 2) {
        // Not enough data — provide a basic fallback instead of silent empty
        const fallback: LearningPath = {
          focus_topic: 'Khởi động cơ bản',
          reason: 'Chưa có đủ dữ liệu học tập để cá nhân hóa. Hãy học một vài từ vựng trước!',
          steps: [
            { type: 'warmup', label: 'Khởi động', icon: '🌸', prompt: 'Hướng dẫn mình ôn lại những từ đã học nhé!' },
            { type: 'practice', label: 'Luyện tập', icon: '⚡', prompt: 'Cho mình luyện tập thêm với các từ mới!' },
            { type: 'challenge', label: 'Thử thách', icon: '🏆', prompt: 'Thử thách mình với câu hỏi khó hơn đi!' },
          ],
