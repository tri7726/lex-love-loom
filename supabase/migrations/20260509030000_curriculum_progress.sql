...
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can view their own curriculum progress') THEN
        CREATE POLICY "Users can view their own curriculum progress"
            ON public.curriculum_progress FOR SELECT
            USING (auth.uid() = user_id);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can insert their own curriculum progress') THEN
