...
serve(async (req: Request) => {
  ...
  try {
    ...
    const rl = await checkRateLimit(req, supabaseClient, { tier: "medium", endpoint: "sensei-insights" });
    if (rl) return rl;
    ...
    const result = await withCache(
      supabaseClient,
      `sensei_insights:${user_id}`,
      300, // 5 min — analysis is not real-time
      async () => {
        ...
        if (!aiRes.ok) {
          throw new Error(`AI Gateway error (${aiRes.status}): ${JSON.stringify(aiData).slice(0, 300)}`);
        }
        const choices = aiData.choices as Array<Record<string, unknown>>;
        if (!choices || choices.length === 0) {
          throw new Error(`No choices: ${JSON.stringify(aiData).slice(0, 300)}`);
        }
