...
const ALLOWED_ORIGINS = [
  "http://localhost:5173",
  "http://localhost:4173",
  "https://lex-love-loom.vercel.app",
  "https://lex-love-loom.netlify.app",
...
Bạn có khả năng phân tích sâu lỗi sai của người dùng và tạo các thử thách từ vựng/ngữ pháp/phát âm đa dạng.
LUÔN trả về các loại câu hỏi khác nhau trong cùng một skill — không trùng quest_type.

CÁC LOẠI CÂU HỎI (quest_type):
- "multiple-choice": Trắc nghiệm thường, chọn 1 trong 4 đáp án (dịch nghĩa, chọn từ đúng...)
- "particle-fill": Điền trợ từ (は/が/に/で/を/へ) vào chỗ trống ___
- "translation": Dịch câu — cho câu JP, chọn bản dịch VN đúng, kèm phân tích sắc thái (nuance)
- "word-distinction": Phân biệt từ gần nghĩa (VD: 趣旨 vs 目的 vs 意図), kèm bảng so sánh

QUY TẮC PROGRESSION: 4 câu hỏi phải tăng dần độ khó:
1. Câu 1: difficulty="easy" — nhận diện cơ bản
2. Câu 2: difficulty="medium" — áp dụng
3. Câu 3: difficulty="hard" — phân tích
4. Câu 4: difficulty="master" — tổng hợp / dịch thuật / sắc thái

CẤU TRÚC JSON TRẢ VỀ (chỉ json, không markdown):
{
  "title": "Tên nhiệm vụ hấp dẫn (VD: Chinh phục trợ từ に)",
  "description": "Mô tả nhiệm vụ và lý do cần ôn (tiếng Việt, 1-2 câu)",
  "type": "vocabulary | pronunciation | grammar",
  "jlpt_level": "N5 | N4 | N3 | N2 | N1",
  "challenge_data": {
    "questions": [
      {
        "quest_type": "multiple-choice | particle-fill | translation | word-distinction",
        "difficulty": "easy | medium | hard | master",
        "question": "Nội dung câu hỏi",
        "options": ["A. lựa chọn", "B. lựa chọn", "C. lựa chọn", "D. lựa chọn"],
        "correct_answer": "phải khớp CHÍNH XÁC một trong 4 options",
        "explanation": "Giải thích chi tiết (tiếng Việt)",
        "nuance": "(chỉ cho translation) Phân tích sắc thái giữa các bản dịch",
        "word_compare": [
          {"word": "từ JP", "meaning": "nghĩa tiếng Việt + khác biệt"}
        ]
      }
    ]
  },
...
QUY TẮC BẮT BUỘC:
- 4 câu hỏi, mỗi câu có quest_type KHÁC NHAU nếu có thể
- Mỗi câu ĐÚNG 4 lựa chọn, correct_answer khớp chính xác 1 trong 4
- difficulty tăng dần: easy → medium → hard → master
- Nội dung giải quyết ĐÚNG điểm yếu, không chung chung
- xp_reward: 50-200
- Chỉ trả về JSON thuần, không markdown`;

// ── Validate AI-generated skill structure before inserting ───────────────
const VALID_QUEST_TYPES = ["multiple-choice", "particle-fill", "translation", "word-distinction"];
...
function validateSkillData(data: Record<string, unknown>): { valid: boolean; error?: string } {
  ...
  for (const q of cd.questions as Record<string, unknown>[]) {
    ...
    if (!q.options.includes(q.correct_answer)) {
      return { valid: false, error: `correct_answer "${q.correct_answer}" must exactly match one of the options` };
    }
    if (!q.explanation) return { valid: false, error: "Question missing explanation" };
    // quest_type defaults to 'multiple-choice' if not provided
    if (q.quest_type && !VALID_QUEST_TYPES.includes(q.quest_type as string)) {
      return { valid: false, error: `Invalid quest_type: ${q.quest_type}` };
    }
  ...
  if (data.jlpt_level && !VALID_JLPT.includes(data.jlpt_level as string)) {
    return { valid: false, error: `Invalid jlpt_level: ${data.jlpt_level}` };
  }
...
serve(async (req: Request) => {
  ...
  try {
    ...
    // Estimate JLPT level from flashcard data
    let estimatedJLPT = "N4";
    if (flashcards.length > 0) {
      const jlptLevels = flashcards.map((f: { jlpt_level?: string }) => f.jlpt_level || "N5").filter(Boolean);
      if (jlptLevels.length > 0) {
        // Pick the highest (hardest) JLPT level in user's flashcards
        const levelOrder = ["N5", "N4", "N3", "N2", "N1"];
        estimatedJLPT = jlptLevels.reduce((max: string, l: string) =>
          levelOrder.indexOf(l) > levelOrder.indexOf(max) ? l : max, "N5"
        );
      }
    }
    ...
    if (flashcards.length === 0 && mistakeList.length === 0) {
      context = `Người dùng đang học tốt và chưa có điểm yếu rõ ràng. Hãy tạo một Skill Quest ôn tập từ vựng tổng hợp thú vị.
Hoạt động gần đây: ${JSON.stringify(recentActivity?.slice(0, 5) || [])}.
Đề nghị type="${preferredType}", jlpt_level="${estimatedJLPT}", xp_reward=${suggestedXp}.
Yêu cầu: 4 câu hỏi với 4 quest_type KHÁC NHAU (multiple-choice, particle-fill, translation, word-distinction).`;
    } else {
...
- jlpt_level="${estimatedJLPT}" (dựa trên dữ liệu học của user)
- Đề nghị xp_reward=${suggestedXp}
- 4 câu hỏi với 4 quest_type KHÁC NHAU (multiple-choice, particle-fill, translation, word-distinction)
- difficulty tăng dần: easy → medium → hard → master`;
    }
    ...
    const { data: insertedSkill, error: insertError } = await supabaseClient
      ...
      .insert({
        ...
        jlpt_level: (skillData.jlpt_level as string) || estimatedJLPT,
